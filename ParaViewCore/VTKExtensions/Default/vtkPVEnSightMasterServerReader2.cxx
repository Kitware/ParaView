...
#else
template <class T>
int vtkPVEnSightMasterServerReader2SyncValues(T*,
    int,
    int,
    vtkMultiProcessController*)
  {
    ...
    vtkInformationVector ** vtkNotUsed(inputVector), vtkInformationVector *outputVector)
{
  ...
  // of pieces.
  if((vtkPVEnSightMasterServerReader2SyncValues(parseResults, 2,
  ...
  // Across nodes.
  this->Internal->EnSightVersion
      = this->Internal->RealReaders[0]->GetEnSightVersion();

  if(vtkPVEnSightMasterServerReader2SyncValues(&this->Internal->EnSightVersion,
      1, this->Controller->GetNumberOfProcesses(), this->Controller) != VTK_OK)
    {
  ...
  vtkDataArrayCollection* timeSets =
      this->Internal->RealReaders[0]->GetTimeSets();
  ...
  // Across nodes.
  this->Internal->NumberOfTimeSets = (timeSets ? timeSets->GetNumberOfItems()
      : 0);
  ...
  for(i = 0; i < this->Internal->NumberOfTimeSets; ++i)
    ...
    for(unsigned int rIdx = 1; rIdx < this->Internal->RealReaders.size(); rIdx++)
      {
      pTimeSets = this->Internal->RealReaders[rIdx]->GetTimeSets();
      if(timeSets->GetItem(i)->GetNumberOfTuples()
          != pTimeSets->GetItem(i)->GetNumberOfTuples())
        {
...
//----------------------------------------------------------------------------
static int vtkPVEnSightMasterServerReader2StartsWith(const char* str1,
    const char* str2)
{
...
int vtkPVEnSightMasterServerReader2::ParseMasterServerFile()
{
  ...
  while(vtkPVEnSightMasterServerReader2GetLineFromStream(fin, line))
    ...
    if(!readingFormat && vtkPVEnSightMasterServerReader2StartsWith(
        line.c_str(), "FORMAT"))
      {
    ...
    // and case file names from the SOS file.
    else if(!readingServers && vtkPVEnSightMasterServerReader2StartsWith(
        line.c_str(), "SERVERS"))
      {
      ...
      readingServers = 1;
        ...
        && vtkPVEnSightMasterServerReader2StartsWith(line.c_str(),
            "number of servers:"))
      {
...
{
  return this->Internal->RealReaders.size() == 0 ? 0
...
{
  return this->Internal->RealReaders.size() == 0 ? NULL
