...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader::ReadGeometryFile(const char* fileName, int timeStep,
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader::ReadScalarsPerElement(const char* fileName,
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader::ReadVectorsPerElement(const char* fileName,
                                                  const char* description,
                                                  int timeStep,
                                                  vtkMultiBlockDataSet *compositeOutput)
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader::CreateUnstructuredGridOutput(int partId,
                                                         char line[256],
                                                         const char* name,
                                                         vtkMultiBlockDataSet *compositeOutput)
{
    ...
    if (strncmp(line, "coordinates", 11) == 0)
      ...
      if (isdigit(subLine[0]))
        ...
        for (i = 0; i < numElements; i++)
          ...
          this->InsertNextCellAndId(output,VTK_VERTEX,1,nodeIds,idx,vtkPEnSightReader::POINT,i,numElements);
          lineRead = this->ReadNextDataLine(line);
          }
        }
      else
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_LINE,2,nodeIds,idx,vtkPEnSightReader::BAR2,i,numElements);
        lineRead = this->ReadNextDataLine(line);
        }
      delete [] nodeIds;
      delete [] intIds;
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_QUADRATIC_EDGE,3,nodeIds,idx,vtkPEnSightReader::BAR3,i,numElements);
        lineRead = this->ReadNextDataLine(line);
        }
      delete [] nodeIds;
      delete [] intIds;
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_POLYGON, numNodesPerElement[i],nodeIds,idx,vtkPEnSightReader::NSIDED,i,numElements);
        lineRead = this->ReadNextDataLine(line);
        delete [] nodeIds;
        delete [] intIds;
        }
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_CONVEX_POINT_SET, elementNodeCount,nodeIds,idx,vtkPEnSightReader::NFACED,i,numElements);
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader::CreateStructuredGridOutput(int partId,
                                                       char line[256],
                                                       const char* name,
                                                       vtkMultiBlockDataSet *compositeOutput)
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader::ReadOrSkipCoordinates(vtkPoints* points, long offset,int partId, int* lineRead, char line[256], bool skip)
{
  int numPts,i;
  char subLine[256];
  double point[3];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader::InjectCoordinatesAtEnd(vtkUnstructuredGrid* output, long coordinatesOffset, int partId )
{
  int fakeLineRead;
  char fakeLine[256];
  ios::iostate state;
