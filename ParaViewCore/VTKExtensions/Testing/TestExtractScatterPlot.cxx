...
int TestExtractScatterPlot(int, char*[])
{
  ...
  extraction->SetInputArrayToProcess(1, 0, 0, vtkDataSet::FIELD_ASSOCIATION_POINTS_THEN_CELLS, "Normals");
  ...
  vtkDoubleArray* const y_bin_extents = vtkDoubleArray::SafeDownCast(scatter_plot->GetCellData()->GetArray("y_bin_extents"));
  if(!y_bin_extents)
    {
    return 1;
    }
  ...
  vtkUnsignedLongArray* const bin_values = vtkUnsignedLongArray::SafeDownCast(scatter_plot->GetCellData()->GetArray("bin_values"));
  if(!bin_values)
    {
    return 1;
    }
