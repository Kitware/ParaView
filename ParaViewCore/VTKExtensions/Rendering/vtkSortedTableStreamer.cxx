...
public:
  ...
  virtual int  Extract( vtkTable* input, vtkTable* output,
                        vtkIdType block, vtkIdType blockSize,
                        bool revertOrder) = 0;
  virtual int  Compute( vtkTable* input, vtkTable* output,
                        vtkIdType block, vtkIdType blockSize,
                        bool revertOrder) = 0;
    ...
    if(processId > -1 && mergedTable->GetColumnByName("vtkOriginalProcessIds"))
      ...
      vtkIdTypeArray* processIdArray =
          vtkIdTypeArray::SafeDownCast(
              mergedTable->GetColumnByName("vtkOriginalProcessIds"));
...
public:
  ...
  public:
    ...
    Histogram(int size) : Size(size), TotalValues(0)
      {
      this->Min = 0;
      this->Delta = 0;
      this->Inverted = false;
    ...
    bool CanBeReduced()
      {
      return this->TotalValues != this->Values[0] && (this->Delta > 10E-5);
      }
  ...
  public:
    ...
    void Update(T* dataPtr, vtkIdType numTuples, int numComponents,
                int selectedComponent, vtkIdType histogramSize,
    ...
    void SortProcessId(vtkIdType* dataPtr, vtkIdType numTuples,
                       vtkIdType histogramSize,
                       double* scalarRange, bool reverseOrder)
      {
      // Clear memory if needed
      this->Clear();
...
public:
    ...
    if(!sortableArray)
      ...
      if(this->DataToSort)
        ...
        this->LocalSorter->Update(static_cast<T*>(this->DataToSort->GetVoidPointer(0)),
                                  this->DataToSort->GetNumberOfTuples(),
      ...
      this->MPI->AllGather(this->LocalSorter->Histo->Values,
                           bufferHistogramValues,
                           HISTOGRAM_SIZE);
    ...
    if(this->Me == mergePid)
      ...
      vtkDataArray* subsetArray =
          vtkDataArray::SafeDownCast(
              localResult->GetColumnByName("vtkOriginalProcessIds"));
      ...
      if(subsetArray)
        ...
        sorter.SortProcessId(static_cast<vtkIdType*>(subsetArray->GetVoidPointer(0)),
                             subsetArray->GetNumberOfTuples(),
                             HISTOGRAM_SIZE,
                             this->CommonRange,
                             revertOrder);

        localResult.TakeReference(
            this->NewSubsetTable(localResult.GetPointer(),
                                 &sorter,
                                 0,
                                 localResult->GetNumberOfRows()));
        }
  ...
  int Compute(vtkTable* input, vtkTable* output,
    ...
    this->SearchGlobalIndexLocation( (block * blockSize),
                                     this->LocalSorter->Histo,
                                     this->GlobalHistogram,
                                     nbElementsToRemoveFromHead,
                                     localOffset,
                                     nbElementsInBar);
    ...
    vtkIdType searchIdx =
        (this->GlobalHistogram->TotalValues < (block + 1) * blockSize) ?
        this->GlobalHistogram->TotalValues : ((block + 1) * blockSize);
    searchIdx--; // It is not a size it is an index (so -1)

    this->SearchGlobalIndexLocation( searchIdx,
                                     this->LocalSorter->Histo,
                                     this->GlobalHistogram,
                                     globalUpperOffset,
                                     upperOffset,
    ...
    localSubset.TakeReference( this->NewSubsetTable( input,
                                                     this->LocalSorter,
                                                     localOffset,
                                                     localSize));
    ...
    if( this->Me == mergePid)
      ...
      vtkDataArray* subsetArray = vtkDataArray::SafeDownCast(
          localSubset->GetColumnByName(this->DataToSort->GetName()));
      ...
      sorter.Update(static_cast<T*>(subsetArray->GetVoidPointer(0)),
                    subsetArray->GetNumberOfTuples(),
                    subsetArray->GetNumberOfComponents(),
                    this->SelectedComponent,
                    HISTOGRAM_SIZE,
                    this->CommonRange,
                    revertOrder);
      ...
      localSubset.TakeReference(
          this->NewSubsetTable(localSubset.GetPointer(),
                               &sorter,
                               nbElementsToRemoveFromHead,
                               blockSize));
  ...
  void SearchGlobalIndexLocation(vtkIdType searchedGlobalIndex,
                                 Histogram* localHistogram,
                                 Histogram* globalHistogram,
                                 vtkIdType& nbGlobalToSkip,
                                 vtkIdType& localOffset,
    ...
    do
      ...
      nbInLocalBar = _localHistogram.GetNumberOfElements(histogramBarIdx,
                                                         histogramBarIdx+1);
      ...
      this->MPI->AllGather( _localHistogram.Values,
                            bufferHistogramValues, HISTOGRAM_SIZE);
  ...
  // --------------------------------------------------------------------------
  static vtkTable* NewSubsetTable( vtkTable* srcTable,
                            ArraySorter* sorter,
                            vtkIdType offset,
                            vtkIdType size)
    {
    ...
    if (input->GetFieldData()->GetArray("STRUCTURED_DIMENSIONS"))
      ...
      if( output )
        ...
        vtkIdTypeArray* idsArray =
            vtkIdTypeArray::SafeDownCast(
                output->GetColumnByName("vtkOriginalIndices"));
        vtkIdTypeArray* pidsArray =
            vtkIdTypeArray::SafeDownCast(
                output->GetColumnByName("vtkOriginalProcessIds"));
...
//            {
//            cout << "Error invalid pid " << pid << " at idx " << idx << " mergePid " << mergePid << " me " << this->Me << endl;
//            WaitForGDB();
//            }

          structuredIndices->InsertNextTuple3(
              (id % dimensions[3*pid]),
  ...
  bool TestInternalClasses()
  {
    ...
    for(int i=0;i<2048;i++) // 2048 is bigger than histogram
      ...
      dataB->InsertNextTuple3(vtkMath::Random(),
                              vtkMath::Random(),
                              vtkMath::Random());
      }
    ...
    if(histPart1.TotalValues + histPart2.TotalValues != dataA->GetNumberOfTuples())
      {
      cout << "Invalid number of elements in the histogram. Expected "
    ...
    sortedArray.Update(static_cast<T*>(dataA->GetVoidPointer(0)),
                       dataA->GetNumberOfTuples(),
                       dataA->GetNumberOfComponents(),
                       0,
                       100,
                       dataA->GetRange(),
                       false);
    ...
    if( sortedArray.ArraySize != dataA->GetNumberOfTuples())
      {
      cout << "Invalide sorted array size. Expected "
          << dataA->GetNumberOfTuples() << " and got "
    ...
    if( sortedArray.Array[0].Value != min )
      {
      cout << "The min is not the first element in the array. Expected: "
          << min << " and got " << sortedArray.Array[0].Value << endl;
      return false;
      }

    if( sortedArray.Array[sortedArray.ArraySize-1].Value != max )
      {
      cout << "The max is not the first element in the array. Expected: "
          << max << " and got " << sortedArray.Array[sortedArray.ArraySize-1].Value << endl;
      return false;
      }
    ...
    if( sortedArray.Array[0].Value != max )
      {
      cout << "The max is not the first element in the array. Expected: "
          << max << " and got " << sortedArray.Array[0].Value << endl;
      return false;
      }
    ...
    if( sortedArray.Array[sortedArray.ArraySize-1].Value != min )
      {
      cout << "The min is not the first element in the array. Expected: "
          << min << " and got " << sortedArray.Array[sortedArray.ArraySize-1].Value << endl;
      return false;
      }

    cout << "ArraySorter ok [" << dataA->GetRange()[0] << ", "
         << dataA->GetRange()[1] << "]" << endl;
                                           ...
                                           vtkInformationVector* outputVector)
{
  ...
  if(!input)
    ...
    for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); blockIdx++,
         iter->GoToNextItem())
      {
      vtkTable* other = 0;
      if((other = vtkTable::SafeDownCast(iter->GetCurrentDataObject())))
        ...
        if(iter->GetCurrentMetaData()->Has(vtkSelectionNode::HIERARCHICAL_LEVEL()) &&
          ...
          for(int i=0;i<other->GetNumberOfRows();i++)
            {
            compositeIndex->InsertNextTuple1(
                static_cast<unsigned int>(
                    iter->GetCurrentMetaData()->Get(
  ...
  int realComponent = (!arrayToProcess) ?  0 :
                      this->GetSelectedComponent() % arrayToProcess->GetNumberOfComponents();
  ...
  if(!this->Internal)
    ...
    if(data)
      ...
      switch (data->GetDataType())
        {
        vtkTemplateMacro(
            this->Internal = new Internals<VTK_TT>( input, data,
                                                    this->GetController());
        );
        default:
      ...
      this->Internal = new Internals<double>( input, 0,
                                              this->GetController());
      }
    }
}
...
void vtkSortedTableStreamer::PrintInfo(vtkTable* input)
{
  ...
  stream << "Process " << vtkMultiProcessController::GetGlobalController()->GetLocalProcessId() << endl
