...
public:
  ...
  // Description:
  // Expand bounding box to contain the string's bounding box.
  void DrawMathTextString(float* point, const vtkStdString& string);
  ...
  void SetColor4(unsigned char color[4]);

  // Description:
  // Implement pure virtual member function. Does not affect bounding rect.
  void SetTexture(vtkImageData* image, int properties);
  ...
  void DrawPoly(float* points, int n, unsigned char* colors = 0, int nc_comps = 0);

  // Description:
  // Expand bounding rect to contain the given lines.
  void DrawLines(float* f, int n, unsigned char* colors = 0, int nc_comps = 0);

  // Description:
  // Expand bounding rect to contain the given points.
  void DrawPoints(float* points, int n, unsigned char* colors = 0, int nc_comps = 0);

  // Description:
  // Expand bounding rect to contain the point sprites.
  void DrawPointSprites(
    vtkImageData* sprite, float* points, int n, unsigned char* colors = 0, int nc_comps = 0);

  // Description:
  // Expand bounding rect to contain the markers.
  void DrawMarkers(
    int shape, bool highlight, float* points, int n, unsigned char* colors = 0, int nc_comps = 0);
