...
public:
  ...
  Data() : CompressTime(0), DecompressTime(0), CompressedSize(0)
  {
  }
};
...
int TestImageCompressors(int argc, char* argv[])
{
  ...
  vtkSmartPointer<vtkUnsignedCharArray> input = vtkUnsignedCharArray::SafeDownCast(image->GetPointData()->GetScalars());
  ...
  cout << "Input: "
    << image->GetDimensions()[0] << "x"
    << image->GetDimensions()[1] << "x"
  ...
  for (MapType::iterator iter = datas.begin(); iter != datas.end(); ++ iter)
    ...
    cout << iter->first.c_str() << " :"
      << " compress: "
      << (iter->second.CompressTime / max_count)
      << " decompress: "
      << (iter->second.DecompressTime / max_count)
      << " compression ratio: "
      << ( (uncompressedSize - iter->second.CompressedSize) * 100.0 / uncompressedSize)
      << "( compressed size: " << iter->second.CompressedSize << ")"
      << endl;
    }
  return TEST_SUCCESS;
}
