...
namespace
{
  static inline vtkIdType FindCell(
    vtkImageData* image, double point[3])
    {
    double pcoords[3]; int subid=0;
    return image->vtkImageData::FindCell(
      point, NULL, -1, 0.1, subid, pcoords, NULL);
    }
}
  ...
  vtkOverlappingAMR* amr)
{
  ...
  if (something_changed)
    ...
    for (int cc=0; cc < this->ResampledAMR->GetCellData()->GetNumberOfArrays();
      cc++)
      {
...
bool vtkResampledAMRImageSource::Initialize(vtkOverlappingAMR* amr)
{
  ...
  vtkStreamingStatusMacro(
    "resampled image resolution: " << dimensions[0] << ", " <<
  ...
  for (unsigned int level=0; reference == NULL &&
    level < amr->GetNumberOfLevels(); level++)
    {
    for (unsigned int index=0; reference == NULL &&
      index < amr->GetNumberOfDataSets(level); index++)
      {
  ...
  dualGrid->SetDimensions(
    output->GetDimensions()[0]-1,
    output->GetDimensions()[1]-1,
    output->GetDimensions()[2]-1);
  dualGrid->SetOrigin(
    output->GetOrigin()[0] + output->GetSpacing()[0] / 2.0,
  ...
  for (double z = updateBounds.GetMinPoint()[2] ;
    ...
    for (double y = updateBounds.GetMinPoint()[1];
      ...
      for (double x = updateBounds.GetMinPoint()[0];
        ...
        if (receiver_cellid == -1 ||
          receiver_cellid >= this->DonorLevel->GetNumberOfTuples()||
