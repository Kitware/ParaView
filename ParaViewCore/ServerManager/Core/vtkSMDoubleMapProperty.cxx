...
void vtkSMDoubleMapProperty::WriteTo(vtkSMMessage* msg)
{
  ...
  for(iter = this->Private->Map.begin();
      iter != this->Private->Map.end();
      iter++)
    {
    variant->add_idtype(iter->first);

    std::vector<double>::iterator double_iter;
    for(double_iter = iter->second.begin();
        double_iter != iter->second.end();
        double_iter++)
      {
...
//---------------------------------------------------------------------------
void vtkSMDoubleMapProperty::ReadFrom(const vtkSMMessage *msg,
                                      int offset,
                                      vtkSMProxyLocator* locator)
{
  ...
  const ProxyState_Property *prop =
    &msg->GetExtension(ProxyState::property, offset);
...
//---------------------------------------------------------------------------
int vtkSMDoubleMapProperty::ReadXMLAttributes(vtkSMProxy* parent,
