...
vtkSMProxy* vtkSMPreselectionPipeline::GetOrCreateSelectionRepresentation()
{
  vtkSMProxy* proxyISelectionRepresentation =
    this->GetSelectionRepresentation();
  if (!proxyISelectionRepresentation)
    {
    proxyISelectionRepresentation =
      this->CreateSelectionRepresentation(NULL);
    }
  ...
  vtkSMSourceProxy* selection, vtkSMRenderViewProxy* view)
{
  if (representation && view)
    ...
    if (this->SelectionRepresentation && view->GetSession() != this->SelectionRepresentation->GetSession())
      {
      this->ClearCache();
      }
