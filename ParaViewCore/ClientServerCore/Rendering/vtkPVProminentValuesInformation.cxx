...
void vtkPVProminentValuesInformation::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  if(this->DistinctValues)
    {
    for ( vtkInternalDistinctValues::iterator cit = this->DistinctValues->begin(); cit != this->DistinctValues->end(); ++ cit )
      {
      os << i2 << "Component " << cit->first << " (" << cit->second.size() << " values )" << endl;
      for ( vtkInternalDistinctValues::mapped_type::iterator eit = cit->second.begin(); eit != cit->second.end(); ++ eit )
        {
        os << i3;
...
//----------------------------------------------------------------------------
void vtkPVProminentValuesInformation::DeepCopy(
...
int vtkPVProminentValuesInformation::Compare(vtkPVProminentValuesInformation *info)
{
  ...
  if (
    strcmp(info->GetFieldName(), this->FieldName) == 0 &&
...
//----------------------------------------------------------------------------
void vtkPVProminentValuesInformation::DeepCopyParameters(
  vtkPVProminentValuesInformation* other)
{
...
//----------------------------------------------------------------------------
void vtkPVProminentValuesInformation::CopyFromCompositeDataSet(
...
//----------------------------------------------------------------------------
void vtkPVProminentValuesInformation::CopyFromLeafDataObject(
  vtkDataObject* dobj)
{
  if (!dobj)
    {
...
void vtkPVProminentValuesInformation::CopyDistinctValuesFromObject(vtkAbstractArray* array)
{
  ...
  for (int c = (nc > 1 ? -1 : 0); c < nc; ++c)
    ...
    std::set<std::vector<vtkVariant> >& compDistincts(
      (*this->DistinctValues)[c]);
    cvalues->Initialize();
...
void vtkPVProminentValuesInformation::CopyToStream(vtkClientServerStream* css)
{
  ...
  *css
  ...
  int numberOfDistinctValueComponents = static_cast<int>(
    this->DistinctValues ? this->DistinctValues->size() : 0);
...
void vtkPVProminentValuesInformation::CopyParametersToStream(vtkMultiProcessStream& mps)
{
  ...
  mps
    << magic_number << this->PortNumber
    << std::string(this->FieldAssociation) << std::string(this->FieldName) << this->NumberOfComponents
...
void vtkPVProminentValuesInformation::CopyParametersFromStream(vtkMultiProcessStream& mps)
{
  ...
  mps
    >> magic_number >> this->PortNumber
    >> fieldAssoc >> fieldName >> this->NumberOfComponents
    >> this->Fraction >> this->Uncertainty;
...
//-----------------------------------------------------------------------------
void vtkPVProminentValuesInformation::AddDistinctValues(
  vtkPVProminentValuesInformation* info)
{
  ...
  for ( int i = 0; i <= this->NumberOfComponents; ++ i )
    ...
    vtkInternalDistinctValues::mapped_type::iterator eit; // iterator over entries of component [ab]it->second.
    bool tooManyValues = false;
    if (
      ait == this->DistinctValues->end() ||
      bit == info->DistinctValues->end() )
      ...
      for (eit = bit->second.begin(); eit != bit->second.end(); ++eit)
        {
        if (
          (*this->DistinctValues)[i].insert( *eit ).second &&
...
//-----------------------------------------------------------------------------
vtkAbstractArray* vtkPVProminentValuesInformation::GetProminentComponentValues(
  int component)
{
