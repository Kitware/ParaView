...
This section is for developers who want to implement a restarted output reader or a reader for any other time series of files. Reading a time series of files is handled by the `vtkFileSeriesReader` class (located in the `VTKExtensions/IOCore/` directory). `vtkFileSeriesReader` is really a meta-reader that takes a “core” reader that will do the actual loading and parsing of data from files. In order to use `vtkFileSeriesReader`, you must place it in a special `FileSeriesReaderProxy`, which will provide the introspection facilities needed to use the core reader.
