...
proc tablelist::createAuxObject {win key row col aux auxType auxWidth} {
	...
	#
	label $aux -borderwidth 0 -height 0 -highlightthickness 0 \
	...
	#
	frame $aux -borderwidth 0 -container 0 \
...
proc tablelist::createSeps win {
	...
	frame $w -background $data(-background) -borderwidth 1 -container 0 \
		 -cursor $data(-cursor) -highlightthickness 0 -relief sunken \
		 -takefocus 0 -width 2
...
proc tablelist::adjustSeps win {
	...
	$w configure -height [expr {$sepHeight + [winfo height $data(hdr)] - 1}]
	place $w -in $data(hdrTxtFrLbl)$col -anchor ne -bordermode outside \
		 -relx 1.0 -x 1 -y 1
	raise $w
    }
	...
	if {[regexp {^sep[0-9]+$} [winfo name $w]]} {
	    ...
	    if {$data(-showlabels)} {
		place configure $w -rely 1.0 -y -1
	    } else {
...
proc tablelist::adjustColumns {win whichWidths stretchCols} {
	...
	if {$col == $data(editCol) &&
	    [string compare [winfo class $data(bodyFrEd)] "Checkbutton"] != 0} {
...
proc tablelist::adjustLabel {win col pixels alignment} {
	...
	switch $alignment {
		...
		if {[string compare $text ""] == 0} {
		    ...
		    place $w.il -anchor nw -relx 0.5 -x $iX -relheight 1.0
		    place $w.tl -anchor ne -relx 0.5 -x $tX -relheight 1.0
		}
	    }
	}
    }
...
proc tablelist::adjustElidedText win {
	...
	#
	# Update btmTextIdx because it may change due to the "elidedCol" tag
	#
	set btmTextIdx [$w index @0,$height]
    }
	...
	for {set line [expr {int($btmTextIdx)}]} \
	    ...
	    #
	    # Update topTextIdx because it may change due to the "elidedCol" tag
	    #
	    set topTextIdx [$w index @0,0]
	}
    }
