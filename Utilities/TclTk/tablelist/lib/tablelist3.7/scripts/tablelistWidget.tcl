...
namespace eval tablelist {
	...
	insertlist itemlistvar labelpath labels move movecolumn nearest \
	...
	separators size sort sortbycolumn sortcolumn sortorder xview yview]
...
proc tablelist::tablelist args {
    ...
    #
    # Create the "stripe", "active", "select", "disabled", and
    # "elidedCol" tags in the body text widget.  Don't use the built-in
    # "sel" tag because on Windows the selection in a text widget
    ...
    #
    frame $data(rowGap) -borderwidth 1 -container 0 -highlightthickness 0 \
			-relief sunken -takefocus 0 -height 4
    frame $data(colGap) -borderwidth 1 -container 0 -highlightthickness 0 \
...
proc tablelist::tablelistWidgetCmd {win argList} {
	...
	entrypath {
		...
		if {[string compare $class "Mentry"] == 0 ||
		    [string compare $class "Checkbutton"] == 0} {
		    return ""
		} else {
...
proc tablelist::seecellSubCmd {win row col} {
	...
	switch $alignment {
		...
		#
		# Shift the view in the body text widget to make the
		# column's start or end position visible, but adjust
		# it again if the first tab has become invisible
		#
		$data(hdrTxt) xview moveto [lindex [$w xview] 0]
		if {[winfo viewable $win]} {
		    update idletasks
		}
		set lX [winfo rootx $data(hdrTxtFrLbl)$col]
		if {$lX < [winfo rootx $w]} {
		    $w xview moveto [expr {double($lX)/$data(hdrPixels)}]
		    while {![isCharVisible $w $tabIdx1]} {
			$w xview scroll 1 units
		    }
		} elseif {![isCharVisible $w $nextIdx]} {
		    while 1 {
		...
		#
		# Shift the view in the body text widget to make the column's
		# start position visible, but adjust it again if the character
		# (\t or \n) at the position nextIdx has become invisible
		#
		$data(hdrTxt) xview moveto [lindex [$w xview] 0]
		if {[winfo viewable $win]} {
		    update idletasks
	...
	if {($col < $data(-titlecolumns)) ||
	    (!$data($col-elided) &&
	     ...
	     $scrlWindowWidth)} {
	    #
	    # The given column index specifies either a title
	    # column or one that is fully visible; bring the cell's
	    # row into the window and restore the horizontal view
	    #
	    set fraction [lindex [$w xview] 0]
	    $w see $textIdx
	    $w xview moveto $fraction
	    adjustElidedTextWhenIdle $win
		  ...
		  [getColWidth $win $col] > $scrlWindowWidth} {
	    #
	    # The given column index specifies either an elided column or
	    # one that doesn't fit into the window; bring the cell's row
	    # into the window and shift the horizontal view to make the
	    # column the first visible one among all scrollable columns
	    #
	    $w see $textIdx
	    set scrlColOffset 0
	    ...
	    #
	    # The given column index specifies a non-elided scrollable
	    # column; bring the cell's row into the window and shift the
	    # horizontal view repeatedly until the column becomes visible
	    #
	    $w see $textIdx
