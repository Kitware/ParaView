...
proc tablelist::doConfig {win opt val} {
	...
	b {
		...
		-font {
			...
			set key [lindex $item end]
			    ...
			    $data(bodyFrEd) configure $editWin($name-fontOpt) \
	...
	w {
		...
		-titlecolumns {
			...
			$data(hdrTxt) configure -xscrollcommand ""
			if {$oldVal == 0} {
			    frame $w -background $data(-foreground) \
...
proc tablelist::doColConfig {col win opt val} {
	...
	-editwindow {
	    variable editWin
	    if {[regexp {^(entry|checkbutton)$} $val] ||
		($::tk_version >= 8.4 &&
		 [string compare $val "spinbox"] == 0) ||
	...
	-selectforeground {
		...
		if {!$data($col-hide)} {
		    ...
		    # column in all (partly) selected rows
		    #
...
proc tablelist::doRowConfig {row win opt val} {
	...
	-selectforeground {
		...
		#
		# Configure the tag $tag in the body text widget and
		# apply it to the selected elements of the given row
		#
		set optTail [string range $opt 7 end]	;# remove the -select
		$w tag configure $tag -$optTail $val
		$w tag lower $tag disabled
...
proc tablelist::doCellConfig {row col win opt val} {
	...
	-image {
		...
		if {[winfo exists $aux] &&
		    ...
		    set keepAux 0
		    destroy $aux		;# no harm if aux doesn't exist
		    ...
		    label $aux -borderwidth 0 -height 0 -highlightthickness 0 \
			       -image $val -padx 0 -pady 0 -relief flat \
			       -takefocus 0
