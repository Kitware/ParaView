...
The MPAS reader now can read the primary grid from MPAS files. MPAS files
...
Although the simulation operates mostly on the primary grid, the MPAS reader
...
the MPAS reader now has a **Use Primary Grid** option that allows users to see the
original primary grid. Multilayers are also supported by extruding general
polygons into polyhedra cell types.

That said, loading the dual grid (**Use Primary Grid** off) is still the
default, which maintains backward compatibility with the previous behavior. The
