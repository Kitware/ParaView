  ...
  pqRepresentation* dataContext, const QStringList& context) const
{
    ...
    menu->addAction(QIcon(":/pqWidgets/Icons/pqEyeballClosed.svg"), tr("Hide Block"),
      [=]()
      {
        ...
        if (!prevValues.empty() && prevValues[0] != "/" && prevValues[0] != rootName)
        {
          ...
          // So what we do is we put all strings from smProperty in a
          // std::unordered_set<std::string>. We use this dictionary to do fast queries from the
          // list of blocks that should be displayed (which are all in the `value` container). If a
          // string is not in smProperty but is in value, then it was hidden in the past, so we
  ...
  {
    menu->addAction(QIcon(":/pqWidgets/Icons/explicit_opacity.png"), tr("Set Block Opacity"),
      [=]()
      {
    ...
    menu->addAction(QIcon(":/pqWidgets/Icons/inherited_opacity.png"), tr("Unset Block Opacity"),
      [=]()
      {
        SM_SCOPED_TRACE(PropertiesModified).arg("proxy", reprProxy);
