...
template<GridType gridType>
Grid<gridType>::Grid() : 
  MpiRank(-1), ChunkCapacity(0), NCells2d(0),
  NLon(0), NLat(0), NLev(0),
  Lev(NULL),
  Radius(0),
  Ne(0), Np(0), 
  LonStep(0), LatStep(0),
...
void Grid<gridType>::MinFaceIndex(int side, int index[3]) const
{
  ...
  double indexD[3] = {static_cast<double>(index[0]),
...
template<GridType gridType>
size_t Grid<gridType>::CubeToIndex (
  ...
  double v[3], std::vector<std::vector<int> >& index) const
{
  ...
  for (int i = 0; i < NFACES; ++i)
    ...
    if (eq(v[valIndex[i].index[0]], valIndex[i].value))
      ...
      int oneIndex[3] = {
        static_cast<int>(it1 - cubeCoordinates.begin()),
        static_cast<int>(it2 - cubeCoordinates.begin()),
        valIndex[i].face};
