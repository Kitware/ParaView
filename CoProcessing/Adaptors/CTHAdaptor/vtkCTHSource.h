...
public:
  ...
  void Initialize(int igm, int n_blocks, int nmat, int max_mat, 
                  int NCFields, int NMFields, double *x0, double *x1,
                  int max_level);
  // Description:
  // Initialize block parameters for block_id
  void InitializeBlock(int block_id, 
           int Nx, int Ny, int Nz, 
           double* x, double* y, double* z,
           int active, int active1, int level);
  // Description:
  // Initialize field parmaeters for field_id
  void SetCellFieldName(int field_id, char *field_name, 
  ...
  void SetCellFieldPointer(int block_id, int field_id, 
                           int k, int j, double *istrip);
  void SetMaterialFieldPointer(int block_id, int field_id, int mat, 
                               int k, int j, double *istrip);
  // Description:
  // Update block parameters for block_id (due to refinement/coarsening)
  void UpdateBlock(int block_id, int active, int active1, 
           int level, int max_level,
           int bxbot, int bxtop, 
           int bybot, int bytop, 
           int bzbot, int bztop, 
           int* neighb_proc, int *neighb_block);
