...
void checkARLType(int i, int aType, char *Id, int count, FILE *fp)
{
  static char tmp[256];
  int is_array = 0;
  int n_size = 0;
  ...
  if (aType == 309)
    {
    fprintf(fp,"&&\n      msg->ArgumentTypes[%i] == vtkARLStream::vtk_object_pointer ",i+2);
    return;
    }
  
  if (aType%1000 == 303)
    {
    fprintf(fp,"&&\n      msg->ArgumentTypes[%i] == vtkARLStream::string_value ",i+2);
    return;
    }
...
/*    strcat(tmp,"_array"); */
  ...
  fprintf(fp," &&\n      msg->ArgumentTypes[%i] == vtkARLStream::%s",i+2,tmp);
  if (is_array)
    {
...
void use_hints(FILE *fp)
{
  ...
  switch (currentFunction->ReturnType%1000)
    ...
    case 313: case 314: case 315: case 316:
      fprintf(fp,"    *resultStream << vtkARLStream::Reply << vtkARLStream::InsertArray(temp%i,%i) << vtkARLStream::End;\n", MAX_ARGS, currentFunction->HintSize);
      break;
    }
}
...
void return_result(FILE *fp)
{
  switch (currentFunction->ReturnType%1000)
    ...
    case 13: case 14: case 15: case 16:
      fprintf(fp,"    *resultStream << vtkARLStream::Reply << temp%i << vtkARLStream::End;\n",
              MAX_ARGS); 
      break;
    case 303:
      fprintf(fp,"    if (temp%i)\n      {\n      *resultStream << vtkARLStream::Reply << temp%i << vtkARLStream::End;\n",MAX_ARGS,MAX_ARGS); 
      fprintf(fp,"      }\n");
      break;
    case 109:
    case 309:  
      fprintf(fp,"    *resultStream << vtkARLStream::Reply << (vtkObject *)temp%i << vtkARLStream::End;\n",MAX_ARGS);
      break;
...
void get_args(FILE *fp, int i)
{
  ...
  switch (currentFunction->ArgTypes[i]%1000)
    ...
    case 16:
      ...
      break;      
    case 303:
      fprintf(fp,"    temp%i = vtkARLUtil::GetString(msg,%i);\n",
              i,start_arg);
      break;
    case 109:
    case 309:
...
void outputFunction(FILE *fp, FileInfo *data)
{
  ...
  if (args_ok && 
    ...
    for (i = 0; i < currentFunction->NumberOfArguments; ++i)
      {
      checkARLType(i, currentFunction->ArgTypes[i],
  ...
  if (!strcmp("vtkObject",data->ClassName))
    ...
    fprintf(fp,"      msg->ArgumentTypes[2] == vtkARLStream::string_value)\n");
    fprintf(fp,"    {\n");
    fprintf(fp,"    vtkARLProgressObserver *apo = vtkARLProgressObserver::New();\n");
    fprintf(fp,"    vtkObject* obj = arlu->GetObjectFromMessage(msg, 0, 1);\n");
    fprintf(fp,"    apo->SetFilterID(arlu->GetIDFromObject(obj));\n");
    fprintf(fp,"    apo->SetARLUtil(arlu);\n");
    fprintf(fp,"    char *temp0 = vtkARLUtil::GetString(msg,2);\n");
...
void vtkParseOutput(FILE *fp, FileInfo *data)
{
  ...
  fprintf(fp,"// ARL wrapper for %s object\n//\n",data->ClassName);
  fprintf(fp,"#include \"vtkSystemIncludes.h\"\n");
  fprintf(fp,"#include \"%s.h\"\n",data->ClassName);
  fprintf(fp,"#include \"vtkARLUtil.h\"\n");
  fprintf(fp,"#include \"vtkARLStream.h\"\n\n");
  fprintf(fp,"#include \"vtkARLMessage.h\"\n\n");
  fprintf(fp,"#include \"vtkARLArrayInformation.h\"\n\n");
  if (!strcmp("vtkObject",data->ClassName))
    {
    fprintf(fp,"#include \"vtkARLProgressObserver.h\"\n\n");
    }
  if (data->IsConcrete)
    {
  ...
  for (i = 0; i < data->NumberOfSuperClasses; i++)
    {
      {
      fprintf(fp,"int %sCommand(vtkARLUtil *, vtkObject *, const char *, vtkARLMessage *, vtkARLStream* resultStream);\n",data->SuperClasses[i]);
      }
    }
  
  fprintf(fp,"\nint VTKARL_EXPORT %sCommand(vtkARLUtil *arlu, vtkObject *ob, const char *method, vtkARLMessage *msg, vtkARLStream* resultStream)\n{\n",data->ClassName);
  ...
  /*fprintf(fp,"  vtkARLStream resultStream;\n");*/
