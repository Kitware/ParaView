/*=========================================================================
  ...
  Module:    vtkPVMesaRenderWindow.cxx
  Language:  C++
  Date:      $Date$
  Version:   $Revision$
...
void OSMesaDestroyWindow(void *Window) {
  cerr << "OSMesaDestroyWindow\n";
  free(Window);
}

void *OSMesaCreateWindow(int width, int height) {
  cerr << " OSMesaCreateWindow: " << width << "  " << height << "\n";
  return malloc(width*height*4);
}

/*
XVisualInfo *vtkPVMesaRenderWindowTryForVisual(Display *DisplayId,
...
XVisualInfo *vtkPVMesaRenderWindow::GetDesiredVisualInfo()
{
  XVisualInfo   *v = NULL;
  int           multi;
  int           stereo = 0;
  ...
  for (stereo = this->StereoCapableWindow; !v && stereo >= 0; stereo--)
	...
	XFree(v);
      ...
      v = vtkPVMesaRenderWindowTryForVisual(this->DisplayId,
					    this->DoubleBuffer, 
					    stereo, multi);
...
static int vtkPVMesaRenderWindowGlobalMaximumNumberOfMultiSamples = 8;

void vtkPVMesaRenderWindow::SetGlobalMaximumNumberOfMultiSamples(int val)
{
  if (val == vtkPVMesaRenderWindowGlobalMaximumNumberOfMultiSamples) return;
  vtkPVMesaRenderWindowGlobalMaximumNumberOfMultiSamples = val;
}

int vtkPVMesaRenderWindow::GetGlobalMaximumNumberOfMultiSamples()
{
  return vtkPVMesaRenderWindowGlobalMaximumNumberOfMultiSamples;
}

vtkPVMesaRenderWindow::vtkPVMesaRenderWindow()
{
  //  this->DisplayId = NULL;
  this->ContextId = NULL;
  this->MultiSamples = vtkPVMesaRenderWindowGlobalMaximumNumberOfMultiSamples;
  this->WindowId = NULL;
...
vtkPVMesaRenderWindow::~vtkPVMesaRenderWindow()
{
  ...
  if (this->ContextId)
    ...
    OSMesaDestroyContext( this->ContextId);
    this->ContextId = NULL;

    // then close the old window 
    //    if (this->OwnWindow && this->DisplayId && this->WindowId)
    if (this->OwnWindow && this->WindowId)
      {
	//      XDestroyWindow(this->DisplayId,this->WindowId);
	OSMesaDestroyWindow(this->WindowId);
	this->WindowId = NULL;
      }
    }
...
void vtkPVMesaRenderWindow::Start(void)
{
...
void vtkPVMesaRenderWindow::Frame(void)
{
  glFlush();
...
void vtkPVMesaRenderWindow::StereoUpdate(void)
{
...
//
void vtkPVMesaRenderWindow::SetStereoCapableWindow(int capable)
{
  if (!this->WindowId)
    {
...
void vtkPVMesaRenderWindow::WindowConfigure()
{
...
void vtkPVMesaRenderWindow::WindowInitialize (void)
{
  ...
  */
  ...
  if (!this->WindowId)
    {
      this->WindowId = OSMesaCreateWindow(width,height);
      this->OwnWindow = 1;
    }
  assert(this->WindowId);

  this->ContextId = OSMesaCreateContext(GL_RGBA, NULL);
  assert(this->ContextId);
  this->MakeCurrent();
...
void vtkPVMesaRenderWindow::Initialize (void)
{
...
void vtkPVMesaRenderWindow::SetFullScreen(int arg)
{ /*
  int *temp;
...
void vtkPVMesaRenderWindow::PrefFullScreen()
{ /*
  int *size;
...
// Resize the window.
void vtkPVMesaRenderWindow::WindowRemap()
{
  short cur_light;
...
/*
void vtkPVMesaRenderWindow::SetPosition(int x,int y)
{
...
void vtkPVMesaRenderWindow::SetSize(int x,int y)
{
  ...
  //XSync(this->DisplayId,False);
  OSMesaDestroyWindow(this->WindowId);
  this->WindowId = OSMesaCreateWindow(x,y);
}


/*
int vtkPVMesaRenderWindow::GetDesiredDepth()
{
  XVisualInfo *v;
  int depth = 0;
...
Visual *vtkPVMesaRenderWindow::GetDesiredVisual ()
{
  XVisualInfo *v;
  Visual *vis;
...
Colormap vtkPVMesaRenderWindow::GetDesiredColormap ()
{
  XVisualInfo *v;
...
void vtkPVMesaRenderWindow::PrintSelf(ostream& os, vtkIndent indent)
{
...
unsigned char *vtkPVMesaRenderWindow::GetPixelData(int x1, int y1, 
						   int x2, int y2, 
						   int front)
{
  int     y_low, y_hi;
...
void vtkPVMesaRenderWindow::SetPixelData(int x1, int y1, int x2, int y2,
...
float *vtkPVMesaRenderWindow::GetRGBAPixelData(int x1, int y1, int x2, int y2, int front)
{
  int     y_low, y_hi;
  int     x_low, x_hi;
  int     width, height;
...
void vtkPVMesaRenderWindow::SetRGBAPixelData(int x1, int y1, int x2, int y2,
...
float *vtkPVMesaRenderWindow::GetZbufferData( int x1, int y1, int x2, int y2  )
{
  int             y_low;
  int             x_low;
  int             width, height;
...
void vtkPVMesaRenderWindow::SetZbufferData( int x1, int y1, int x2, int y2,
					  float *buffer )
{
  int             y_low;
  int             x_low;
...
void vtkPVMesaRenderWindow::MakeCurrent()
{
  // set the current window 
  /*
  ...
  if (this->ContextId) {
    if (OSMesaMakeCurrent(this->ContextId, this->WindowId, GL_UNSIGNED_BYTE, this->Size[0], this->Size[1]) != GL_TRUE) {
      assert(0);
    }
  }
}

void vtkPVMesaRenderWindow::RegisterTextureResource (GLuint id)
{
