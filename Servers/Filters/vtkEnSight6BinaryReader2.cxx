/*=========================================================================
  ...
  Module:    vtkEnSight6BinaryReader2.cxx
...
vtkCxxRevisionMacro(vtkEnSight6BinaryReader2, "1.1");
vtkStandardNewMacro(vtkEnSight6BinaryReader2);

//----------------------------------------------------------------------------
vtkEnSight6BinaryReader2::vtkEnSight6BinaryReader2()
{
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::OpenFile(const char* filename)
{
  if (!filename)
    {
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadGeometryFile(const char* fileName,
                                              int timeStep,
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::SkipTimeStep()
{
  char line[80], subLine[80];
  int lineRead;
  int pointIdsListed;
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::SkipStructuredGrid(char line[256])
{
  char subLine[80];
  int lineRead;
  int iblanked = 0;
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::SkipUnstructuredGrid(char line[256])
{
  int lineRead = 1;
  int numElements;
  int cellType;
  ...
  while(lineRead && strncmp(line, "part", 4) != 0)
    ...
    if (strncmp(line, "point", 5) == 0)
      ...
      if (strncmp(line, "tria3", 5) == 0)
        {
        vtkDebugMacro("tria3");
        cellType = vtkEnSightReader2::TRIA3;
        }
      else
        {
        vtkDebugMacro("tria6");
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkEnSightReader2::TRIA6;
        }
      ...
      if (cellType == vtkEnSightReader2::TRIA3)
        {
        this->IFile->seekg((sizeof(int)*3*numElements), ios::cur);
        }
      else
      ...
      if (strncmp(line, "quad8", 5) == 0)
        ...
        vtkDebugMacro("quad8");
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkEnSightReader2::QUAD8;
        }
      else
        {
        vtkDebugMacro("quad4");
        cellType = vtkEnSightReader2::QUAD4;
        }
      ...
      if (cellType == vtkEnSightReader2::QUAD4)
        {
      ...
      if (strncmp(line, "tetra10", 7) == 0)
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkEnSightReader2::TETRA10;
        }
      else
        {
        vtkDebugMacro("tetra4");
        cellType = vtkEnSightReader2::TETRA4;
        }
      ...
      if (cellType == vtkEnSightReader2::TETRA4)
        {
        this->IFile->seekg((sizeof(int)*4*numElements), ios::cur);
        }
      else
      ...
      if (strncmp(line, "pyramid13", 9) == 0)
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkEnSightReader2::PYRAMID13;
        }
      else
        {
        vtkDebugMacro("pyramid5");
        cellType = vtkEnSightReader2::PYRAMID5;
        }
      ...
      if (cellType == vtkEnSightReader2::PYRAMID5)
        {
      ...
      if (strncmp(line, "hexa20", 6) == 0)
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkEnSightReader2::HEXA20;
        }
      else
        {
        vtkDebugMacro("hexa8");
        cellType = vtkEnSightReader2::HEXA8;
        }
      ...
      if (cellType == vtkEnSightReader2::HEXA8)
        {
        this->IFile->seekg((sizeof(int)*8*numElements), ios::cur);
        }
      else
      ...
      if (strncmp(line, "penta15", 7) == 0)
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkEnSightReader2::PENTA15;
        }
      else
        {
        vtkDebugMacro("penta6");
        cellType = vtkEnSightReader2::PENTA6;
        }
      ...
      if (cellType == vtkEnSightReader2::PENTA6)
        {
        this->IFile->seekg((sizeof(int)*6*numElements), ios::cur);
        }
      else
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadMeasuredGeometryFile(
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadScalarsPerNode(
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadVectorsPerNode(
  const char* fileName, const char* description, int timeStep,
  vtkMultiBlockDataSet *compositeOutput, int measured)
{
  char line[80];
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadTensorsPerNode(
  const char* fileName, const char* description, int timeStep,
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadScalarsPerElement(
  const char* fileName, const char* description, int timeStep,
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadVectorsPerElement(
  const char* fileName, const char* description, int timeStep,
  vtkMultiBlockDataSet *compositeOutput)
{
  char line[80];
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::ReadTensorsPerElement(
  const char* fileName, const char* description, int timeStep,
  vtkMultiBlockDataSet *compositeOutput)
{
  char line[80];
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::CreateUnstructuredGridOutput(
  ...
  vtkMultiBlockDataSet *compositeOutput)
{
  ...
  for (i = 0; i < vtkEnSightReader2::NUMBER_OF_ELEMENT_TYPES; i++)
    {
  ...
  while(lineRead && strncmp(line, "part", 4) != 0)
    ...
    if (strncmp(line, "point", 5) == 0)
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->GetCellIds(idx, vtkEnSightReader2::POINT)->InsertNextId(cellId);
        }
      delete [] nodeIds;
      delete [] nodeIdList;
      }
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->GetCellIds(idx, vtkEnSightReader2::BAR2)->InsertNextId(cellId);
        }
      delete [] nodeIds;
      delete [] nodeIdList;
      }
      ...
      for (i = 0; i < numElements; i++)
        ...
        cellId = output->InsertNextCell(VTK_LINE, 2, nodeIds);
        this->GetCellIds(idx, vtkEnSightReader2::BAR3)->InsertNextId(cellId);
        }
      delete [] nodeIds;
      delete [] nodeIdList;
      }
...
//----------------------------------------------------------------------------
int vtkEnSight6BinaryReader2::CreateStructuredGridOutput(
  int partId, char line[80], const char* name,
  vtkMultiBlockDataSet *compositeOutput)
{
  char subLine[80];
...
int vtkEnSight6BinaryReader2::ReadLine(char result[80])
{
...
int vtkEnSight6BinaryReader2::ReadIntNumber(int *result)
{
...
// Returns zero if there was an error.
int vtkEnSight6BinaryReader2::ReadIntArray(int *result,
                                          int numInts)
{
  if (numInts <= 0)
    {
...
// Returns zero if there was an error.
int vtkEnSight6BinaryReader2::ReadFloatArray(float *result,
                                            int numFloats)
{
  if (numFloats <= 0)
    {
...
//----------------------------------------------------------------------------
void vtkEnSight6BinaryReader2::PrintSelf(ostream& os, vtkIndent indent)
{
