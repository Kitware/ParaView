#include "vtkPEnSightGoldBinaryReader2.h"
...
//----------------------------------------------------------------------------
vtkPEnSightGoldBinaryReader2::vtkPEnSightGoldBinaryReader2()
{
  this->IFile = NULL;
  this->FileSize = 0;
  this->Fortran = 0;
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::OpenFile(const char* filename)
{
  if (!filename)
    {
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::InitializeFile(const char* fileName)
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadGeometryFile(const char* fileName, int timeStep,
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::CountTimeSteps()
{
  int count=0;
  while(1)
    {
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::SkipTimeStep()
{
  char line[80], subLine[80];
  int lineRead;
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::SkipStructuredGrid(char line[256])
{
  char subLine[80];
  int lineRead;
  int iblanked = 0;
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::SkipUnstructuredGrid(char line[256])
{
  int lineRead = 1;
  int i;
  int numElements;
  ...
  while(lineRead && strncmp(line, "part", 4) != 0)
    ...
    if (strncmp(line, "coordinates", 11) == 0)
      ...
      //cellType = vtkPEnSightReader2::NSIDED;
      this->ReadInt(&numElements);
      if (numElements < 0 || numElements*(int)sizeof(int) > this->FileSize ||
          numElements > this->FileSize)
        {
      ...
      if (strncmp(line, "tria6", 5) == 0 ||
        ...
        vtkDebugMacro("tria6");
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkPEnSightReader2::TRIA6;
        }
      else
        {
        vtkDebugMacro("tria3");
        cellType = vtkPEnSightReader2::TRIA3;
        }
      ...
      if (cellType == vtkPEnSightReader2::TRIA6)
        {
        // Skip nodeIdList.
      ...
      if (strncmp(line, "quad8", 5) == 0 ||
        ...
        vtkDebugMacro("quad8");
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkPEnSightReader2::QUAD8;
        }
      else
        {
        vtkDebugMacro("quad4");
        cellType = vtkPEnSightReader2::QUAD4;
        }
      ...
      if (cellType == vtkPEnSightReader2::QUAD8)
        {
        // Skip nodeIdList.
      ...
      if (strncmp(line, "tetra10", 7) == 0 ||
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkPEnSightReader2::TETRA10;
        }
      else
        {
        vtkDebugMacro("tetra4");
        cellType = vtkPEnSightReader2::TETRA4;
        }
      ...
      if (cellType == vtkPEnSightReader2::TETRA10)
        {
        // Skip nodeIdList.
      ...
      if (strncmp(line, "pyramid13", 9) == 0 ||
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkPEnSightReader2::PYRAMID13;
        }
      else
        {
        vtkDebugMacro("pyramid5");
        cellType = vtkPEnSightReader2::PYRAMID5;
        }
      ...
      if (cellType == vtkPEnSightReader2::PYRAMID13)
        {
        // Skip nodeIdList.
      ...
      if (strncmp(line, "hexa20", 6) == 0 ||
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkPEnSightReader2::HEXA20;
        }
      else
        {
        vtkDebugMacro("hexa8");
        cellType = vtkPEnSightReader2::HEXA8;
        }
      ...
      if (cellType == vtkPEnSightReader2::HEXA20)
        {
        // Skip nodeIdList.
      ...
      if (strncmp(line, "penta15", 7) == 0 ||
        ...
        vtkWarningMacro("Only vertex nodes of this element will be read.");
        cellType = vtkPEnSightReader2::PENTA15;
        }
      else
        {
        vtkDebugMacro("penta6");
        cellType = vtkPEnSightReader2::PENTA6;
        }
      ...
      if (cellType == vtkPEnSightReader2::PENTA15)
        {
        // Skip nodeIdList.
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::SkipRectilinearGrid(char line[256])
{
  char subLine[80];
  int lineRead;
  int iblanked = 0;
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::SkipImageData(char line[256])
{
  char subLine[80];
  int lineRead;
  int iblanked = 0;
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadMeasuredGeometryFile(const char* fileName,
                                                           int timeStep,
                                                           vtkMultiBlockDataSet *output)
{
  char line[80], subLine[80];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadScalarsPerNode(
                                                     ...
                                                     int numberOfComponents, int component)
{
  ...
  if (measured)
    ...
    if (numPts)
      ...
      for (i = 0; i < numPts; i++)
        {
        this->InsertVariableComponent(scalars,i,component, &(scalarsRead[i]),partId,0,SCALAR_PER_NODE);
        }
  ...
  while (lineRead && strncmp(line, "part", 4) == 0)
    ...
    if (numPts)
      ...
      for (i = 0; i < numPts; i++)
        {
        this->InsertVariableComponent(scalars, i, component,&(scalarsRead[i]), realId, 0, SCALAR_PER_NODE);
        }
      if (component == 0)
        {
        scalars->SetName(description);
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadVectorsPerNode(
                                                     const char* fileName, const char* description, int timeStep,
                                                     vtkMultiBlockDataSet *compositeOutput, int measured)
{
  char line[80];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadTensorsPerNode(
                                                     const char* fileName, const char* description, int timeStep,
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadScalarsPerElement(
                                                        const char* fileName, const char* description, int timeStep,
    ...
    if (numCells)
      ...
      if (strncmp(line, "block", 5) == 0)
        ...
        for (i = 0; i < numCells; i++)
          {
          this->InsertVariableComponent(scalars, i, component, &(scalarsRead[i]), realId, 0, SCALAR_PER_ELEMENT);
          }
        if (this->IFile->eof())
          {
          lineRead = 0;
        ...
        while (lineRead && strncmp(line, "part", 4) != 0 &&
          ...
          for (i = 0; i < numCellsPerElement; i++)
            {
            this->InsertVariableComponent(scalars, i, component, &(scalarsRead[i]), idx, elementType, SCALAR_PER_ELEMENT);
            }
          this->IFile->peek();
          if (this->IFile->eof())
            {
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadVectorsPerElement(
                                                        const char* fileName, const char* description, int timeStep,
                                                        vtkMultiBlockDataSet *compositeOutput)
{
  char line[80];
      ...
      if (strncmp(line, "block", 5) == 0)
        ...
        while (lineRead && strncmp(line, "part", 4) != 0 &&
          ...
          for (i = 0; i < numCellsPerElement; i++)
            ...
            tuple[1] = comp2[i];
            tuple[2] = comp3[i];
            this->InsertVariableComponent(vectors, i, 0, tuple, idx, elementType, VECTOR_PER_ELEMENT);
            }
          this->IFile->peek();
          if (this->IFile->eof())
            {
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadTensorsPerElement(
                                                        const char* fileName, const char* description, int timeStep,
                                                        vtkMultiBlockDataSet *compositeOutput)
{
  char line[80];
      ...
      if (strncmp(line, "block", 5) == 0)
        ...
        while (lineRead && strncmp(line, "part", 4) != 0 &&
          ...
          for (i = 0; i < numCellsPerElement; i++)
            ...
            tuple[4] = comp5[i];
            tuple[5] = comp6[i];
            this->InsertVariableComponent(tensors, i, 0, tuple, idx, elementType, TENSOR_SYMM_PER_ELEMENT);
            }
          this->IFile->peek();
          if (this->IFile->eof())
            {
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::CreateUnstructuredGridOutput(
                                                               ...
                                                               vtkMultiBlockDataSet *compositeOutput)
{
  ...
  for (i = 0; i < vtkPEnSightReader2::NUMBER_OF_ELEMENT_TYPES; i++)
    {
  ...
  while(lineRead && strncmp(line, "part", 4) != 0)
    ...
    if (strncmp(line, "coordinates", 11) == 0)
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_VERTEX,1,nodeIds,idx,vtkPEnSightReader2::POINT,i,numElements);
        }
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_LINE,2,nodeIds,idx,vtkPEnSightReader2::BAR2,i,numElements);
        }
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_QUADRATIC_EDGE,3,nodeIds,idx,vtkPEnSightReader2::BAR3,i,numElements);
        }
      ...
      for (i = 0; i < numElements; i++)
        ...
        this->InsertNextCellAndId(output,VTK_POLYGON, numNodesPerElement[i],nodeIds,idx,cellType,i,numElements);
      ...
      for (i = 0; i < numElements; i++)
        ...
        if (cellType == vtkPEnSightReader2::TRIA6)
          ...
          this->InsertNextCellAndId(output,VTK_QUADRATIC_TRIANGLE, 6,nodeIds,idx,cellType,i,numElements);
          }
        else
          {
          for (j = 0; j < 3; j++)
      ...
      for (i = 0; i < numElements; i++)
        ...
        if (cellType == vtkPEnSightReader2::QUAD8)
          ...
          this->InsertNextCellAndId(output,VTK_QUADRATIC_QUAD, 8,nodeIds,idx,cellType,i,numElements);
          }
        else
          {
          for (j = 0; j < 4; j++)
      ...
      cellType = vtkPEnSightReader2::NFACED;
      this->ReadInt(&numElements);
      if (numElements < 0 || numElements*(int)sizeof(int) > this->FileSize ||
          numElements > this->FileSize)
        {
      ...
      // For each nfaced...
        ...
        this->InsertNextCellAndId(output,VTK_CONVEX_POINT_SET, elementNodeCount,nodeIds,idx,cellType,i,numElements);
      ...
      for (i = 0; i < numElements; i++)
        ...
        if (cellType == vtkPEnSightReader2::TETRA10)
          ...
          this->InsertNextCellAndId(output,VTK_QUADRATIC_TETRA, 10,nodeIds,idx,cellType,i,numElements);
          }
        else
          {
          for (j = 0; j < 4; j++)
      ...
      for (i = 0; i < numElements; i++)
        ...
        if (cellType == vtkPEnSightReader2::PYRAMID13)
          ...
          this->InsertNextCellAndId(output,VTK_QUADRATIC_PYRAMID, 13,nodeIds,idx,cellType,i,numElements);
          }
        else
          {
          for (j = 0; j < 5; j++)
      ...
      for (i = 0; i < numElements; i++)
        ...
        if (cellType == vtkPEnSightReader2::HEXA20)
          ...
          this->InsertNextCellAndId(output,VTK_QUADRATIC_HEXAHEDRON, 20,nodeIds,idx,cellType,i,numElements);
          }
        else
          {
          for (j = 0; j < 8; j++)
          ...
          this->InsertNextCellAndId(output,VTK_HEXAHEDRON,8,nodeIds,idx,cellType,i,numElements);
          }
        }
      delete [] nodeIds;
      delete [] nodeIdList;
      ...
      for (i = 0; i < numElements; i++)
        ...
        if (cellType == vtkPEnSightReader2::PENTA15)
          ...
          this->InsertNextCellAndId(output,VTK_QUADRATIC_WEDGE, 15,nodeIds,idx,cellType,i,numElements);
          }
        else
          {
          for (j = 0; j < 6; j++)
      ...
      if( this->CoordinatesAtEnd && (this->InjectCoordinatesAtEnd(output, coordinatesOffset, idx) == -1) )
        {
        return -1;
        }
      return 1;
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::CreateStructuredGridOutput(
                                                             int partId, char line[80], const char* name,
                                                             vtkMultiBlockDataSet *compositeOutput)
{
  char subLine[80];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::CreateRectilinearGridOutput(
                                                              int partId, char line[80], const char* name,
                                                              vtkMultiBlockDataSet *compositeOutput)
{
  char subLine[80];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::CreateImageDataOutput(
                                                        int partId, char line[80], const char* name,
                                                        vtkMultiBlockDataSet *compositeOutput)
{
  char subLine[80];
  ...
  // Compute new origin
  float newOrigin[3];
  newOrigin[ splitDimension ] = origin[ splitDimension ] + ((float) splitDimensionBeginIndex) * delta[ splitDimension ];
...
int vtkPEnSightGoldBinaryReader2::ReadLine(char result[80])
{
...
int vtkPEnSightGoldBinaryReader2::ReadPartId(int *result)
{
...
// Internal function to read a single integer.
// Returns zero if there was an error.
int vtkPEnSightGoldBinaryReader2::ReadInt(int *result)
{
  char dummy[4];
  if (this->Fortran)
    {
...
// Returns zero if there was an error.
int vtkPEnSightGoldBinaryReader2::ReadIntArray(int *result,
                                               int numInts)
{
  if (numInts <= 0)
    {
...
// Returns zero if there was an error.
int vtkPEnSightGoldBinaryReader2::ReadFloatArray(float *result,
                                                 int numFloats)
{
  if (numFloats <= 0)
    {
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::ReadOrSkipCoordinates(vtkPoints* points, long offset,int partId, bool skip)
{
  int numPts;
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldBinaryReader2::InjectCoordinatesAtEnd(vtkUnstructuredGrid* output, long coordinatesOffset, int partId )
{
  bool eof = false;
...
//----------------------------------------------------------------------------
void vtkPEnSightGoldBinaryReader2::GetVectorFromFloatBuffer(int i, float *vector)
{
...
//----------------------------------------------------------------------------
void vtkPEnSightGoldBinaryReader2::UpdateFloatBuffer()
{
  ...
  for(int i = 0; i < 3; i++)
    ...
    if( this->Fortran )
      this->IFile->seekg(this->FloatBufferFilePosition + 4 + i * (this->FloatBufferNumberOfVectors * sizeof(float) + 8) + this->FloatBufferIndexBegin * sizeof(float) );
    else
      this->IFile->seekg(this->FloatBufferFilePosition + i * this->FloatBufferNumberOfVectors * sizeof(float) + this->FloatBufferIndexBegin * sizeof(float) );

    if (this->IFile->read((char*)this->FloatBuffer[i], sizeof(float)*sizeToRead) == 0)
      {
      vtkErrorMacro("Read failed");
      }
...
//----------------------------------------------------------------------------
void vtkPEnSightGoldBinaryReader2::PrintSelf(ostream& os, vtkIndent indent)
{
