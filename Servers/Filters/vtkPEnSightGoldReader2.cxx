...
//----------------------------------------------------------------------------
vtkPEnSightGoldReader2::vtkPEnSightGoldReader2()
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadGeometryFile(const char* fileName, int timeStep,
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadMeasuredGeometryFile(
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadScalarsPerNode(const char* fileName, const char* description,
    ...
    if (numPts)
      ...
      for (i = 0; i < numLines; i++)
        ...
        for (j = 0; j < 6; j++)
          ...
          this->InsertVariableComponent(scalars,i*6 + j,component, &(scalarsRead[j]),partId,0,SCALAR_PER_NODE);
          }
        this->ReadNextDataLine(line);
        }
      strcpy(formatLine, "");
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadVectorsPerNode(const char* fileName, const char* description,
                                               int timeStep, vtkMultiBlockDataSet *compositeOutput,
                                               int measured)
{
  char line[256], formatLine[256], tempLine[256];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadTensorsPerNode(const char* fileName, const char* description,
                                               int timeStep, vtkMultiBlockDataSet *compositeOutput)
{
  char line[256];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadScalarsPerElement(const char* fileName,
    ...
    if (numCells)
      ...
      if (strncmp(line, "block",5) == 0)
        ...
        for (i = 0; i < numCells; i++)
          ...
          this->InsertVariableComponent(scalars, i, component, &scalar, realId, 0, SCALAR_PER_ELEMENT);
          }
        ...
        while (lineRead && strncmp(line, "part",4) != 0 &&
          ...
          if( partial )
            ...
            for (i = 0; i < numCellsPerElement; i++)
              ...
              this->InsertVariableComponent(scalars, i, component, &scalar, idx, elementType, SCALAR_PER_ELEMENT);
              }
            }
          else
            {
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadVectorsPerElement(const char* fileName,
                                                  const char* description,
                                                  int timeStep,
                                                  vtkMultiBlockDataSet *compositeOutput)
{
        ...
        while (lineRead && strncmp(line, "part",4) != 0 &&
          ...
          for (i = 0; i < 3; i++)
            ...
            for (j = 0; j < numCellsPerElement; j++)
              ...
              //                         i, value);
              this->InsertVariableComponent(vectors, j, i, &value, idx, elementType, SCALAR_PER_ELEMENT);
              }
            }
          lineRead = this->ReadNextDataLine(line);
          } // end while
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadTensorsPerElement(const char* fileName,
                                                  const char* description,
                                                  int timeStep,
                                                  vtkMultiBlockDataSet *compositeOutput)
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::CreateUnstructuredGridOutput(int partId,
                                                         char line[256],
                                                         const char* name,
                                                         vtkMultiBlockDataSet *compositeOutput)
{
  ...
  while(lineRead && strncmp(line, "part", 4) != 0)
    ...
    if (strncmp(line, "coordinates", 11) == 0)
      ...
      int pointsRead = this->ReadOrSkipCoordinates(points, coordinatesOffset,idx, &lineRead, line, true);
      if( pointsRead == -1 )
        return -1;
      points->Delete();
      ...
      if (isdigit(subLine[0]))
        ...
        for (i = 0; i < numElements; i++)
          ...
          //this->GetCellIds(idx, vtkPEnSightReader2::POINT)->InsertNextId(cellId);
      ...
      for (i = 0; i < numElements; i++)
        ...
        //this->GetCellIds(idx, vtkPEnSightReader2::BAR2)->InsertNextId(cellId);
      ...
      for (i = 0; i < numElements; i++)
        ...
        //this->GetCellIds(idx, vtkPEnSightReader2::BAR3)->InsertNextId(cellId);
      ...
      for (i = 0; i < numElements; i++)
        ...
        //this->GetCellIds(idx, vtkPEnSightReader2::NSIDED)->InsertNextId(cellId);
        this->InsertNextCellAndId(output,VTK_POLYGON, numNodesPerElement[i],nodeIds,idx,vtkPEnSightReader2::NSIDED,i,numElements);
        lineRead = this->ReadNextDataLine(line);
        delete [] nodeIds;
        delete [] intIds;
        }
      ...
      for (i = 0; i < numElements; i++)
        ...
        /*cellId = output->InsertNextCell(VTK_CONVEX_POINT_SET,
          elementNodeCount,
          nodeIds);
          this->GetCellIds(idx, vtkPEnSightReader2::NFACED)->InsertNextId(cellId);*/
        this->InsertNextCellAndId(output,VTK_CONVEX_POINT_SET, elementNodeCount,nodeIds,idx,vtkPEnSightReader2::NFACED,i,numElements);
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::CreateStructuredGridOutput(int partId,
                                                       char line[256],
                                                       const char* name,
                                                       vtkMultiBlockDataSet *compositeOutput)
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::CreateRectilinearGridOutput(int partId,
                                                        char line[256],
                                                        const char* name,
                                                        vtkMultiBlockDataSet *compositeOutput)
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::CreateImageDataOutput(int partId,
                                                  char line[256],
                                                  const char* name,
                                                  vtkMultiBlockDataSet *compositeOutput)
{
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::CheckForUndefOrPartial(const char *line)
{
  char undefvar[16];
  ...
  if( r == 1)
    ...
    if( strcmp(undefvar, "undef") == 0 )
      ...
      switch( this->GetSectionType(line) )
        {
        case vtkPEnSightReader2::COORDINATES:
          this->UndefPartial->UndefCoordinates = val;
          break;
        case vtkPEnSightReader2::BLOCK:
          this->UndefPartial->UndefBlock = val;
          break;
        case vtkPEnSightReader2::ELEMENT:
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::ReadOrSkipCoordinates(vtkPoints* points, long offset,int partId, int* lineRead, char line[256], bool skip)
{
  int numPts,i;
  char subLine[256];
  double point[3];
...
//----------------------------------------------------------------------------
int vtkPEnSightGoldReader2::InjectCoordinatesAtEnd(vtkUnstructuredGrid* output, long coordinatesOffset, int partId )
{
  int fakeLineRead;
  char fakeLine[256];
  ios::iostate state;
  ...
  vtkPoints* points = vtkPoints::New();
  int pointsRead = this->ReadOrSkipCoordinates(points, coordinatesOffset,partId, &fakeLineRead, fakeLine, false);
...
//----------------------------------------------------------------------------
void vtkPEnSightGoldReader2::PrintSelf(ostream& os, vtkIndent indent)
{
