/*=========================================================================
  ...
  Module:    vtkCTHFragmentIdList.cxx
...
int search(
      vtkCTHFragmentIdListItem *idList,
      int l,
      int r,
      vtkCTHFragmentIdListItem &itemToFind)
{
  assert(l<=r);
  int m=(r+l)/2;
  // found it.
...
//
void vtkCTHFragmentIdList::Initialize(vector<int> ids, bool preSorted)
{
  // Prep.
  this->Clear();
...
//
int vtkCTHFragmentIdList::GetLocalId(int globalId)
{
  assert("The object must be intialized before querries are made."
         && this->IsInitialized);

  const int firstListItem=0;
  const int lastListItem=this->IdList.size()-1;
  vtkCTHFragmentIdListItem itemToFind(globalId);
  return
