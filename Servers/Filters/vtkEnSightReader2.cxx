...
//----------------------------------------------------------------------------
vtkEnSightReader2::vtkEnSightReader2()
{
...
//----------------------------------------------------------------------------
int vtkEnSightReader2::RequestData(
...
//----------------------------------------------------------------------------
int vtkEnSightReader2::RequestInformation(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **vtkNotUsed(inputVector),
  vtkInformationVector *outputVector)
{
  ...
  while(lineRead &&
    ...
    if (strncmp(line, "constant", 8) == 0)
      ...
      if (strcmp(subLine, "node:") == 0)
        ...
        this->VariableMode = vtkEnSightReader2::SCALAR_PER_NODE;
        ...
        this->VariableMode = vtkEnSightReader2::SCALAR_PER_ELEMENT;
        if (sscanf(line, " %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                   subLine) == 3)
          {
          this->VariableTimeSetIds->InsertNextId(timeSet);
        ...
        this->VariableMode = vtkEnSightReader2::SCALAR_PER_MEASURED_NODE;
      ...
      if (strcmp(subLine, "node:") == 0)
        ...
        this->VariableMode = vtkEnSightReader2::VECTOR_PER_NODE;
        if (sscanf(line, " %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                   subLine) == 3)
          {
          this->VariableTimeSetIds->InsertNextId(timeSet);
        ...
        this->VariableMode = vtkEnSightReader2::VECTOR_PER_ELEMENT;
        if (sscanf(line, " %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                   subLine) == 3)
          {
          this->VariableTimeSetIds->InsertNextId(timeSet);
        ...
        this->VariableMode = vtkEnSightReader2::VECTOR_PER_MEASURED_NODE;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                   subLine) == 3)
          {
          this->VariableTimeSetIds->InsertNextId(timeSet);
      ...
      if (strcmp(subLine, "node:") == 0)
        ...
        this->VariableMode = vtkEnSightReader2::TENSOR_SYMM_PER_NODE;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                   subLine) == 3)
          {
          this->VariableTimeSetIds->InsertNextId(timeSet);
        ...
        this->VariableMode = vtkEnSightReader2::TENSOR_SYMM_PER_ELEMENT;
        if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                   subLine) == 3)
          {
          this->VariableTimeSetIds->InsertNextId(timeSet);
      ...
      if (strcmp(subLine, "scalar") == 0)
        ...
        if (strcmp(subLine, "node:") == 0)
          ...
          this->VariableMode = vtkEnSightReader2::COMPLEX_SCALAR_PER_NODE;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                     subLine) == 3)
            {
          ...
          this->VariableMode = vtkEnSightReader2::COMPLEX_SCALAR_PER_ELEMENT;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                     subLine) == 3)
            {
            this->ComplexVariableTimeSetIds->InsertNextId(timeSet);
        ...
        if (strcmp(subLine, "node:") == 0)
          ...
          this->VariableMode = vtkEnSightReader2::COMPLEX_VECTOR_PER_NODE;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                     subLine) == 3)
            {
            this->ComplexVariableTimeSetIds->InsertNextId(timeSet);
          ...
          this->VariableMode = vtkEnSightReader2::COMPLEX_VECTOR_PER_ELEMENT;
          if (sscanf(line, " %*s %*s %*s %*s %d %d %s", &timeSet, &fileSet,
                     subLine) == 3)
            {
            this->ComplexVariableTimeSetIds->InsertNextId(timeSet);
...
//----------------------------------------------------------------------------
int vtkEnSightReader2::ReadVariableFiles(vtkMultiBlockDataSet *output)
{
  int i, j;
...
//----------------------------------------------------------------------------
void vtkEnSightReader2::AddVariableFileName(const char* fileName1,
...
//----------------------------------------------------------------------------
void vtkEnSightReader2::AddVariableDescription(const char* description)
{
  int size;
  int i;
...
//----------------------------------------------------------------------------
void vtkEnSightReader2::AddVariableType()
{
  int size;
  int i;
  int *types = NULL;
...
//----------------------------------------------------------------------------
int vtkEnSightReader2::GetSectionType(const char *line)
{
  if (strncmp(line, "coordinates", 5) == 0)
    {
    return vtkEnSightReader2::COORDINATES;
    }
  else if (strncmp(line, "block", 4) == 0)
    {
    return vtkEnSightReader2::BLOCK;
    }
  else if (this->GetElementType(line) != -1)
    {
    return vtkEnSightReader2::ELEMENT;
    }
  else
    {
    return -1;
...
//----------------------------------------------------------------------------
int vtkEnSightReader2::GetElementType(const char* line)
{
  if (strncmp(line, "point", 5) == 0)
    {
    return vtkEnSightReader2::POINT;
    }
  else if (strncmp(line, "bar2", 4) == 0)
    {
    return vtkEnSightReader2::BAR2;
    }
  else if (strncmp(line, "bar3", 4) == 0)
    {
    return vtkEnSightReader2::BAR3;
    }
  else if (strncmp(line, "nsided", 6) == 0)
    {
    return vtkEnSightReader2::NSIDED;
    }
  else if (strncmp(line, "tria3", 5) == 0)
    {
    return vtkEnSightReader2::TRIA3;
    }
  else if (strncmp(line, "tria6", 5) == 0)
    {
    return vtkEnSightReader2::TRIA6;
    }
  else if (strncmp(line, "quad4", 5) == 0)
    {
    return vtkEnSightReader2::QUAD4;
    }
  else if (strncmp(line, "quad8", 5) == 0)
    {
    return vtkEnSightReader2::QUAD8;
    }
  else if (strncmp(line, "nfaced", 6) == 0)
    {
    return vtkEnSightReader2::NFACED;
    }
  else if (strncmp(line, "tetra4", 6) == 0)
    {
    return vtkEnSightReader2::TETRA4;
    }
  else if (strncmp(line, "tetra10", 7) == 0)
    {
    return vtkEnSightReader2::TETRA10;
    }
  else if (strncmp(line, "pyramid5", 8) == 0)
    {
    return vtkEnSightReader2::PYRAMID5;
    }
  else if (strncmp(line, "pyramid13", 9) == 0)
    {
    return vtkEnSightReader2::PYRAMID13;
    }
  else if (strncmp(line, "hexa8", 5) == 0)
    {
    return vtkEnSightReader2::HEXA8;
    }
  else if (strncmp(line, "hexa20", 6) == 0)
    {
    return vtkEnSightReader2::HEXA20;
    }
  else if (strncmp(line, "penta6", 6) == 0)
    {
    return vtkEnSightReader2::PENTA6;
    }
  else if (strncmp(line, "penta15", 7) == 0)
    {
    return vtkEnSightReader2::PENTA15;
    }
  else
    {
    return -1;
...
void vtkEnSightReader2::ReplaceWildcards(char* filename, int num)
{
...
//----------------------------------------------------------------------------
void vtkEnSightReader2::RemoveLeadingBlanks(char *line)
{
  int count = 0;
...
//----------------------------------------------------------------------------
vtkIdList* vtkEnSightReader2::GetCellIds(int index, int cellType)
{
  ...
  if(!this->CellIds)
    {
    this->CellIds = new vtkEnSightReader2CellIdsType;
    }
...
//----------------------------------------------------------------------------
void vtkEnSightReader2::AddToBlock(vtkMultiBlockDataSet* output,
...
//----------------------------------------------------------------------------
vtkDataSet* vtkEnSightReader2::GetDataSetFromBlock(
...
//----------------------------------------------------------------------------
void vtkEnSightReader2::SetBlockName(vtkMultiBlockDataSet* output,
...
//----------------------------------------------------------------------------
void vtkEnSightReader2::PrintSelf(ostream& os, vtkIndent indent)
{
