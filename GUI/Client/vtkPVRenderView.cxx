...
#define VTK_PV_USE_LIGHT_KIT_REG_KEY "UseLightKit"
#define VTK_PV_ENABLE_DEFAULT_LIGHT_REG_KEY "EnableDefaultLight"
#define VTK_PV_DEFAULT_LIGHT_INTENSITY_REG_KEY "DefaultLightIntensity"
#define VTK_PV_MAINTAIN_LUMINANCE_REG_KEY "MaintainLuminance"

#define VTK_PV_DEFAULT_BACKGROUND_RED 0.33
#define VTK_PV_DEFAULT_BACKGROUND_GREEN 0.35
#define VTK_PV_DEFAULT_BACKGROUND_BLUE 0.43
/* 
...
void vtkPVRenderView::CreateViewProperties()
{
  ...
  if (rgb[0] == -1)
    {
    rgb[0] = VTK_PV_DEFAULT_BACKGROUND_RED;
    rgb[1] = VTK_PV_DEFAULT_BACKGROUND_GREEN;
    rgb[2] = VTK_PV_DEFAULT_BACKGROUND_BLUE;
    }
  ...
  */
  float intensity = 1.0;
  if (this->GetApplication()->GetRegistryValue(2, "RunTime", 
      VTK_PV_DEFAULT_LIGHT_INTENSITY_REG_KEY, 0))
    {
    intensity = this->GetApplication()->GetFloatRegistryValue(2, "RunTime", 
      VTK_PV_DEFAULT_LIGHT_INTENSITY_REG_KEY);
    }
  ...
  this->SetDefaultLightIntensity(intensity);
  ...
  */
  ...
  if (!pvapp->GetRegistryValue(2, "RunTime", VTK_PV_ENABLE_DEFAULT_LIGHT_REG_KEY, 0) ||
    pvapp->GetIntRegistryValue(2, "RunTime", VTK_PV_ENABLE_DEFAULT_LIGHT_REG_KEY) == 1)
    {
    this->SetDefaultLightSwitch(1);
    }
  else
    {
    this->SetDefaultLightSwitch(0);
    }
  ...
  if (!pvapp->GetRegistryValue(2, "RunTime", VTK_PV_USE_LIGHT_KIT_REG_KEY, 0) ||
    pvapp->GetIntRegistryValue(2, "RunTime", VTK_PV_USE_LIGHT_KIT_REG_KEY) == 0 )
    {
    this->SetUseLight(0);
    }
  else
  ...
  for ( cc = 0; cc < 4; cc ++ )
    ...
    reg_value_valid = 0;
    if (pvapp->GetRegistryValue(2, "RunTime", p.c_str(),0))
      {
      reg_value_valid = 1;
      reg_value = pvapp->GetFloatRegistryValue(2, "RunTime", p.c_str());
      }
    ...
    if (reg_value_valid)
      {
      this->SetLight(vtkLightKit::TKeyLight, subtype, reg_value);
      }
    ...
    if (reg_value_valid)
      {
      this->SetLight(vtkLightKit::TFillLight, subtype, reg_value);
      }
    ...
    if (reg_value_valid)
      {
      this->SetLight(vtkLightKit::TBackLight, subtype, reg_value);
      }
    ...
    if( cc < 2 )
      ...
      if (reg_value_valid)
        {
        this->SetLight(vtkLightKit::THeadLight, subtype, reg_value);
        }
  ...
  if (!pvapp->GetRegistryValue(2, "RunTime", 
      VTK_PV_MAINTAIN_LUMINANCE_REG_KEY, 0) || pvapp->GetIntRegistryValue(2,
      "RunTime", VTK_PV_MAINTAIN_LUMINANCE_REG_KEY) == 0)
    {
    this->SetMaintainLuminance(0);
    }
  else
    {
    this->SetMaintainLuminance(1);
    }
  ...
  // The reset button.
  this->ResetButton->SetParent(this->GeneralPropertiesFrame->GetFrame());
  this->ResetButton->Create();
  this->ResetButton->SetText("Reset");
  this->ResetButton->SetWidth(20);
  this->ResetButton->SetBalloonHelpString("Reset all the settings on the General, Annotate and Camera tabs.");
  this->ResetButton->SetCommand(this, "ResetSettingsToDefault");
  this->Script("pack %s -padx 2 -pady 2 -fill none -expand no -anchor e",
...
void vtkPVRenderView::SetDefaultLightIntensity(double intensity)
{
  ...
  this->EventuallyRender();
  this->GetTraceHelper()->AddEntry("$kw(%s) SetDefaultLightIntensity %f", 
    this->GetTclName(), intensity);
}

//----------------------------------------------------------------------------
void vtkPVRenderView::SetDefaultLightIntensityNoTrace(double intensity)
{
  this->DefaultLightIntensity->SetValue(intensity);
  DoubleVectSetElement(this->RenderModuleProxy->GetProperty("LightIntensity"), 
    intensity);
  this->RenderModuleProxy->UpdateVTKObjects();

  // Save in registry.
  this->GetApplication()->SetRegistryValue(2, "RunTime",
    VTK_PV_DEFAULT_LIGHT_INTENSITY_REG_KEY, "%f", intensity);
}
...
void vtkPVRenderView::SetDefaultLightAmbientColor(double r, double g, double b)
{
  vtkSMDoubleVectorProperty *dp = vtkSMDoubleVectorProperty::SafeDownCast(
    this->RenderModuleProxy->GetProperty("LightAmbientColor"));
...
//----------------------------------------------------------------------------
void vtkPVRenderView::SetDefaultLightSwitchNoTrace(int val)
{
  this->DefaultLightSwitch->SetSelectedState(val);
  ...
  // Add to registry.
  this->GetApplication()->SetRegistryValue(2, "RunTime", 
    VTK_PV_ENABLE_DEFAULT_LIGHT_REG_KEY,  (val? "1" : "0"));
}

//----------------------------------------------------------------------------
void vtkPVRenderView::SetDefaultLightSwitch(int val)
{
  this->SetDefaultLightSwitchNoTrace(val);
  this->GetTraceHelper()->AddEntry("$kw(%s) SetDefaultLightSwitch %d", 
    this->GetTclName(), val); 
}
...
void vtkPVRenderView::SetLightNoTrace(int type, int subtype, double value)
{
  ...
  // Save in reg.
  this->GetApplication()->SetRegistryValue(2, "RunTime", s.c_str(), "%f", value);
...
{
  this->SetMaintainLuminanceNoTrace(s);
...
//----------------------------------------------------------------------------
void vtkPVRenderView::SetMaintainLuminanceNoTrace(int s)
{
  // Set the GUI
  ...
  this->GetApplication()->SetRegistryValue(2, "RunTime", 
    VTK_PV_MAINTAIN_LUMINANCE_REG_KEY, (s? "1" : "0"));
}
...
//----------------------------------------------------------------------------
void vtkPVRenderView::SetUseLightNoTrace(int s)
{
  // Set the GUI
  ...
  // Save to registry.
  this->GetApplication()->SetRegistryValue(2, "RunTime", VTK_PV_USE_LIGHT_KIT_REG_KEY, 
    (s? "1" : "0"));
}
...
{
  this->SetUseLightNoTrace(s);
  this->GetTraceHelper()->AddEntry("$kw(%s) SetUseLight %d", 
    this->GetTclName(), s);
}
...
{
  this->GetTraceHelper()->AddEntry("$kw(%s) ResetSettingsToDefault",
                      this->GetTclName());

  this->SetRendererBackgroundColor(
    VTK_PV_DEFAULT_BACKGROUND_RED, VTK_PV_DEFAULT_BACKGROUND_GREEN,
    VTK_PV_DEFAULT_BACKGROUND_BLUE);
  this->ParallelProjectionCheck->SetSelectedState(0);
  this->TriangleStripsCheck->SetSelectedState(0);
  ...
  if (this->RenderModuleUI)
    {
    this->RenderModuleUI->ResetSettingsToDefault();
    }

  this->SetDisplay3DWidgets(1);

  // Ligting settings.
  this->SetDefaultLightIntensityNoTrace(1.0);
  this->SetDefaultLightSwitchNoTrace(1);
  this->SetUseLightNoTrace(0);
  this->SetMaintainLuminanceNoTrace(0);

  // Orientation axes settings.
  this->SetOrientationAxesVisibility(1);
  this->SetOrientationAxesInteractivity(0);
  this->SetOrientationAxesOutlineColor(1.0, 1.0, 1.0);
  this->SetOrientationAxesTextColor(1.0, 1.0, 1.0);
  this->OrientationAxes->SetViewport(0, 0, 0.2, 0.2); 

  // Camera settings.
  this->ManipulatorControl2D->SetCurrentManipulator(0, 0, "Pan");
  this->ManipulatorControl2D->SetCurrentManipulator(1, 0, "Pan");
  this->ManipulatorControl2D->SetCurrentManipulator(2, 0, "Zoom");
  this->ManipulatorControl2D->SetCurrentManipulator(0, 1, "Roll");
  this->ManipulatorControl2D->SetCurrentManipulator(1, 1, "Pan");
  this->ManipulatorControl2D->SetCurrentManipulator(2, 1, "Pan");
  this->ManipulatorControl2D->SetCurrentManipulator(0, 2, "Pan");
  this->ManipulatorControl2D->SetCurrentManipulator(1, 2, "Pan");
  this->ManipulatorControl2D->SetCurrentManipulator(2, 2, "Pan");

  this->ManipulatorControl3D->SetCurrentManipulator(0, 0, "Rotate");
  this->ManipulatorControl3D->SetCurrentManipulator(1, 0, "Pan");
  this->ManipulatorControl3D->SetCurrentManipulator(2, 0, "Zoom");
  this->ManipulatorControl3D->SetCurrentManipulator(0, 1, "Roll");
  this->ManipulatorControl3D->SetCurrentManipulator(1, 1, "Rotate");
  this->ManipulatorControl3D->SetCurrentManipulator(2, 1, "Pan");
  this->ManipulatorControl3D->SetCurrentManipulator(0, 2, "FlyIn");
  this->ManipulatorControl3D->SetCurrentManipulator(1, 2, "Rotate");
  this->ManipulatorControl3D->SetCurrentManipulator(2, 2, "FlyOut");
}
