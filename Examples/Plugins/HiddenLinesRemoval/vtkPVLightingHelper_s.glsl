...
bool needSurfacePositionInEyeCoordinates()
{
  ...
  {
    result = (gl_LightSource[i].diffuse.w != 0.0) && (gl_LightSource[i].position.w != 0.0);
  }
...
void separateSpecularColor(
  gl_MaterialParameters m, vec3 surfacePosEyeCoords, vec3 n, out vec4 cpri, out vec4 csec)
{
  ...
  for (int i = 0; i < gl_MaxLights; i++)
  {
    ...
    if (gl_LightSource[i].position.w != 0.0)
    {
      ...
      att = 1.0 / (gl_LightSource[i].constantAttenuation +
                    gl_LightSource[i].linearAttenuation * sqrt(sqrDistance) +
                    gl_LightSource[i].quadraticAttenuation * sqrDistance);
    }
    else
    {
      att = 1.0;
    ...
    if (att > 0.0)
    {
      ...
      if (spot > 0.0)
      {
        ...
        // scm*scli
        csec += att * spot * m.specular * gl_LightSource[i].specular * shininessFactor;
      }
    }
  }
}
