...
vtkStandardNewMacro(vtkSQTensorGlyph);

// Construct object with scaling on and scale factor 1.0. Eigenvalues are 
// extracted, glyphs are colored with input scalar data, and logarithmic
// scaling is turned off.
vtkSQTensorGlyph::vtkSQTensorGlyph()
{
  this->Scaling = 1;
...
//----------------------------------------------------------------------------
int vtkSQTensorGlyph::RequestUpdateExtent(
...
//----------------------------------------------------------------------------
int vtkSQTensorGlyph::RequestData(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
  ...
  for (inPtId=0; inPtId < numPts; inPtId++)
    ...
    // cerr << "v1=" << xv[0] << ", " << xv[1] << ", " << xv[2] << endl;
    // cerr << "v2=" << yv[0] << ", " << yv[1] << ", " << yv[2] << endl;
    // cerr << "v3=" << zv[0] << ", " << zv[1] << ", " << zv[2] << endl;
    ...
    for (dir=0; dir < numDirs; dir++) 
      ...
      // cerr << "eigne_dir=" << eigen_dir << endl;
      // cerr << "symetric dir=" << symmetric_dir << endl;
      ...
      // old behaviour of vtkSQTensorGlyphs (which only used one dir), 
...
//----------------------------------------------------------------------------
void vtkSQTensorGlyph::SetSourceConnection(int id, vtkAlgorithmOutput* algOutput)
{
  if (id < 0)
    {
...
//----------------------------------------------------------------------------
int vtkSQTensorGlyph::FillInputPortInformation(int port, vtkInformation *info)
{
  if (port == 1)
    {
...
//----------------------------------------------------------------------------
void vtkSQTensorGlyph::PrintSelf(ostream& os, vtkIndent indent)
{
