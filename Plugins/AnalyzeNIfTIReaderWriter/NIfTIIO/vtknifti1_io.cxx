...
/** \file nifti1_io.c
    \brief main collection of nifti1 i/o routines
           ...
           - converted to C++ by Joseph Hennessey, Center for Imaging Science,
...
//----------------------------------------------------------------------------
void vtknifti1_io::PrintSelf(ostream& os, vtkIndent indent)
{
...
*//*--------------------------------------------------------------------*/
void vtknifti1_io::nifti_disp_lib_hist( void )
{
 ...
 * \sa nifti_image_load_bricks, nifti_free_NBL, valid_nifti_brick_list,
       nifti_image_read
*//*----------------------------------------------------------------------*/
nifti_image *vtknifti1_io::nifti_image_read_bricks(const char * hname, int nbricks,
 ...
 * dim[0] = 4.
 *----------------------------------------------------------------------*/
void vtknifti1_io::update_nifti_image_for_brick_list( nifti_image * nim , int nbricks )
{
   int ndim;
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_update_dims_from_array( nifti_image * nim )
{
   int c, ndim;
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_image_load_bricks( nifti_image * nim , int nbricks,
                             const int * blist, nifti_brick_list * NBL )
{
...
*//*--------------------------------------------------------------------*/
void vtknifti1_io::nifti_free_NBL( nifti_brick_list * NBL )
{
   int c;
 ...
 *----------------------------------------------------------------------*/
int vtknifti1_io::nifti_load_NBL_bricks( nifti_image * nim , int * slist, int * sindex,
                                  nifti_brick_list * NBL, znzFile fp )
{
   ...
   test = vtkznzlib::znztell(fp);  /* store current file position */
   if( test < 0 ){
   ...
   for( c = 0; c < NBL->nbricks; c++ ){
       ...
       if( isrc != prev ){
          ...
          if( fposn != (oposn + isrc*NBL->bsize) ){
             ...
             if( vtkznzlib::znzseek(fp, (long)fposn, SEEK_SET) < 0 ){
 ...
 * return 0 on success, -1 on failure
 *----------------------------------------------------------------------*/
int vtknifti1_io::nifti_alloc_NBL_mem(nifti_image * nim, int nbricks,
 ...
 * return 0 on success, -1 on failure
 *----------------------------------------------------------------------*/
int vtknifti1_io::nifti_copynsort(int nbricks, const int * blist, int ** slist,
                           int ** sindex)
{
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::valid_nifti_brick_list(nifti_image * nim , int nbricks,
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_NBL_matches_nim(const nifti_image *nim,
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_disp_matrix_orient( const char * mesg, mat44 mat )
{
   int i, j, k;
...
*//*--------------------------------------------------------------------*/
char *vtknifti1_io::nifti_strdup(const char *str)
{
  char *dup;
...
*//*-------------------------------------------------------------------------*/
char const * vtknifti1_io::nifti_datatype_string( int dt )
{
   switch( dt ){
...
/*! Determine if the datatype code dt is an integer type (1=YES, 0=NO).
    ...
    \sa     NIFTI1_DATATYPES group in nifti1.h
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_is_inttype( int dt )
{
   switch( dt ){
...
*//*-------------------------------------------------------------------------*/
char const *vtknifti1_io::nifti_units_string( int uu )
{
   switch( uu ){
...
*//*-------------------------------------------------------------------------*/
char const *vtknifti1_io::nifti_xform_string( int xx )
{
   switch( xx ){
...
*//*-------------------------------------------------------------------------*/
char const *vtknifti1_io::nifti_intent_string( int ii )
{
   switch( ii ){
...
*//*-------------------------------------------------------------------------*/
char const *vtknifti1_io::nifti_slice_string( int ss )
{
   switch( ss ){
...
*//*-------------------------------------------------------------------------*/
char const *vtknifti1_io::nifti_orientation_string( int ii )
{
   switch( ii ){
...
*//*------------------------------------------------------------------------*/
void vtknifti1_io::nifti_datatype_sizes( int datatype , int *nbyper, int *swapsize )
{
   int nb=0, ss=0 ;
   switch( datatype ){
     case DT_INT8:
...
*//*-------------------------------------------------------------------------*/
mat44 vtknifti1_io::nifti_quatern_to_mat44( float qb, float qc, float qd,
...
/*! Given the 3x4 upper corner of the matrix R, compute the quaternion
   ...
   \see nifti_quatern_to_mat44, nifti_make_orthog_mat44,
       nifti_mat44_to_orientation
*//*-------------------------------------------------------------------------*/
void vtknifti1_io::nifti_mat44_to_quatern( mat44 R ,
...
*//*-------------------------------------------------------------------------*/
mat44 vtknifti1_io::nifti_mat44_inverse( mat44 R )
{
...
/*! Input 9 floats and make an orthgonal mat44 out of them.
   ...
   \sa nifti_quatern_to_mat44, nifti_mat44_to_quatern,
       nifti_mat44_to_orientation
*//*-------------------------------------------------------------------------*/
mat44 vtknifti1_io::nifti_make_orthog_mat44( float r11, float r12, float r13 ,
...
*//*--------------------------------------------------------------------*/
mat33 vtknifti1_io::nifti_mat33_inverse( mat33 R )   /* inverse of 3x3 matrix */
{
...
*//*--------------------------------------------------------------------*/
float vtknifti1_io::nifti_mat33_determ( mat33 R )   /* determinant of 3x3 matrix */
{
   double r11,r12,r13,r21,r22,r23,r31,r32,r33 ;
                                                       /*  INPUT MATRIX:  */
   r11 = R.m[0][0]; r12 = R.m[0][1]; r13 = R.m[0][2];  /* [ r11 r12 r13 ] */
...
*//*--------------------------------------------------------------------*/
float vtknifti1_io::nifti_mat33_rownorm( mat33 A )  /* max row norm of 3x3 matrix */
{
   float r1,r2,r3 ;
...
*//*--------------------------------------------------------------------*/
float vtknifti1_io::nifti_mat33_colnorm( mat33 A )  /* max column norm of 3x3 matrix */
{
   float r1,r2,r3 ;
...
*//*--------------------------------------------------------------------*/
mat33 vtknifti1_io::nifti_mat33_mul( mat33 A , mat33 B )  /* multiply 2 3x3 matrices */
{
   mat33 C ; int i,j ;
   for( i=0 ; i < 3 ; i++ )
    for( j=0 ; j < 3 ; j++ )
...
*//*-------------------------------------------------------------------------*/
mat33 vtknifti1_io::nifti_mat33_polar( mat33 A )
{
   mat33 X , Y , Z ;
...
*//*-------------------------------------------------------------------------*/
void vtknifti1_io::nifti_mat44_to_orientation( mat44 R , int *icod, int *jcod, int *kcod )
{
...
*//*---------------------------------------------------------------------- */
void vtknifti1_io::swap_nifti_header(nifti_1_header *h , int is_nifti )
{
...
*//*---------------------------------------------------------------------- */
int vtknifti1_io::nifti_swap_as_analyze( nifti_analyze75 * h )
{
   if( !h ) return 1;
...
*//*---------------------------------------------------------------------- */
void vtknifti1_io::old_swap_nifti_header(nifti_1_header *h , int is_nifti )
{
...
*//*-------------------------------------------------------------------------*/
int vtknifti1_io::nifti_get_filesize( const char *pathname )
{
...
*//*--------------------------------------------------------------------*/
size_t vtknifti1_io::nifti_get_volsize(const nifti_image *nim)
{
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_fileexists(const char* fname)
{
   znzFile fp;
   fp = vtkznzlib::znzopen( fname , "rb" , 1 ) ;
   if( !znz_isnull(fp) )  { vtkznzlib::znzclose(fp);  return 1; }
   return 0; /* fp is NULL */
}
...
/*! return whether the filename is valid
    ...
    ".myhiddenimage.nii" |  1     |
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_is_complete_filename(const char* fname)
{
   const char * ext;
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_validfilename(const char* fname)
{
   const char * ext;
...
*//*--------------------------------------------------------------------*/
const char * vtknifti1_io::nifti_find_file_extension( const char * name )
{
   const char * ext;
   char extcopy[8];
   int    len;
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_is_gzfile(const char* fname)
{
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_compiled_with_zlib(void)
{
#ifdef HAVE_ZLIB
    return 1;
#else
...
*//*--------------------------------------------------------------------*/
char * vtknifti1_io::nifti_makebasename(const char* fname)
{
   char *basename;
   const char *ext;
...
/*! set nifti's global debug level, for status reporting
    ...
    - 2, 3 : more details
*//*--------------------------------------------------------------------*/
void vtknifti1_io::nifti_set_debug_level( int level )
{
    g_opts.debug = level;
}
...
*//*--------------------------------------------------------------------*/
void vtknifti1_io::nifti_set_skip_blank_ext( int skip )
{
...
/*! set nifti's global allow_upper_fext flag         28 Apr 2009 [rickr]
    ...
    explicitly set to 0 or 1
*//*--------------------------------------------------------------------*/
void vtknifti1_io::nifti_set_allow_upper_fext( int allow )
{
...
*//*-------------------------------------------------------------------*/
char * vtknifti1_io::nifti_findhdrname(const char* fname)
{
...
/*! check current directory for existing image file
    ...
    NB: it allocates memory for the image filename, which should be freed
        when no longer required
*//*---------------------------------------------------------------------*/
char * vtknifti1_io::nifti_findimgname(const char* fname , int nifti_type)
{
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_set_filenames( nifti_image * nim, const char * prefix, int check,
                         int set_byte_order )
{
...
*//*------------------------------------------------------------------------*/
int vtknifti1_io::nifti_type_and_names_match( nifti_image * nim, int show_warn )
{
...
int vtknifti1_io::fileext_compare(const char * test_ext, const char * known_ext)
{
   char caps[8] = "";
   size_t c,len; 
 ...
 * (test as local string, with max length 7) */
int vtknifti1_io::fileext_n_compare(const char * test_ext,
...
int vtknifti1_io::is_uppercase(const char * str)
{
   size_t c;
   int hasupper = 0;
...
int vtknifti1_io::is_mixedcase(const char * str)
{
   size_t c;
...
int vtknifti1_io::make_uppercase(char * str)
{
   size_t c;
 ...
 * else return -1 */
int vtknifti1_io::compare_strlist(const char * str, char ** strlist, int len)
{
   int c;
...
/*! check whether the given type is on the "approved" list
    ...
    \sa NIFTI_FTYPE_* codes in nifti1_io.h
*//*------------------------------------------------------------------------*/
int vtknifti1_io::is_valid_nifti_type( int nifti_type )
{
...
*//*------------------------------------------------------------------------*/
int vtknifti1_io::nifti_is_valid_datatype( int dtype )
{
...
*//*------------------------------------------------------------------------*/
int vtknifti1_io::nifti_set_type_from_names( nifti_image * nim )
{
...
*//*------------------------------------------------------------------------*/
int vtknifti1_io::is_nifti_file( const char *hname )
{
   ...
   fp = vtkznzlib::znzopen( tmpname , "rb" , nifti_is_gzfile(tmpname) ) ;
   free(tmpname);
   if (znz_isnull(fp))                      return -1 ;  /* bad open? */

   /* read header, close file */

   ii = (int)vtkznzlib::znzread( &nhdr , 1 , sizeof(nhdr) , fp ) ;
   vtkznzlib::znzclose( fp ) ;
...
int vtknifti1_io::print_hex_vals( const char * data, int nbytes, FILE * fp )
{
   int c;
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::disp_nifti_1_header( const char * info, const nifti_1_header * hp )
{
   int c;
...
*//*--------------------------------------------------------------------*/
nifti_image* vtknifti1_io::nifti_convert_nhdr2nim(nifti_1_header nhdr,
                                    const char * fname)
{
   int   ii , doswap , ioff ;
 ...
 */
znzFile vtknifti1_io::nifti_image_open(const char * hname, char * opts, nifti_image ** nim)
{
  znzFile fptr=NULL;
  ...
  *nim = nifti_image_read(hname,0);
  ...
  fptr = vtkznzlib::znzopen( (*nim)->iname, opts, nifti_is_gzfile((*nim)->iname) );
...
*//*--------------------------------------------------------------------*/
nifti_1_header * vtknifti1_io::nifti_read_header(const char * hname, int * swapped, int check)
{
   ...
   fp = vtkznzlib::znzopen( hfile, "rb", nifti_is_gzfile(hfile) );
   if( znz_isnull(fp) ){
   ...
   bytes = (int)vtkznzlib::znzread( &nhdr, 1, sizeof(nhdr), fp );
   vtkznzlib::znzclose( fp );                      /* we are done with the file now */
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_hdr_looks_good(const nifti_1_header * hdr)
{
 ...
 *           < 0 : error condition
 *----------------------------------------------------------------------*/
int vtknifti1_io::need_nhdr_swap( short dim0, int hdrsize )
{
...
*/
nifti_image *vtknifti1_io::nifti_image_read( const char *hname , int read_data )
{
   struct nifti_1_header  nhdr ;
   nifti_image           *nim ;
   znzFile                fp ;
   ...
   ii = (int)vtkznzlib::znzread( &nhdr , 1 , sizeof(nhdr) , fp ) ;       /* read the thing */
   ...
   if( nim == NULL ){
      vtkznzlib::znzclose( fp ) ;                                   /* close the file */
      if( g_opts.debug > 0 )
 ...
 *----------------------------------------------------------------------*/
int vtknifti1_io::has_ascii_header( znzFile fp )
{
   char  buf[16];
   int   nread;
   ...
   nread = (int)vtkznzlib::znzread( buf, 1, 12, fp );
   buf[12] = '\0';

   if( nread < 12 ) return -1;

   vtkznzlib::znzrewind(fp);  /* move back to the beginning, and check */
...
/*! nifti_read_ascii_image  - process as a type-3 .nia image file
   ...
   NOTE: this is NOT part of the NIFTI-1 standard
*//*--------------------------------------------------------------------*/
nifti_image * vtknifti1_io::nifti_read_ascii_image(znzFile fp, char *fname, int flen,
                                     int read_data)
{
   nifti_image * nim;
   ...
   vtkznzlib::znzread( sbuf , 1 , slen , fp ) ;
   ...
   if( remain > 4 ){
      ...
      vtkznzlib::znzseek(fp, txt_size, SEEK_SET);
 ...
 *----------------------------------------------------------------------*/
int vtknifti1_io::nifti_read_extensions( nifti_image *nim, znzFile fp, int remain )
{
   ...
   count = (int)vtkznzlib::znzread( extdr.extension, 1, 4, fp ); /* get extender */
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_add_extension(nifti_image *nim, const char * data, int len, int ecode)
{
...
/* nifti_add_exten_to_list     - add a new nifti1_extension to the list
   ...
   return 0 on success, -1 on error (and free the entire list)
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_add_exten_to_list( nifti1_extension *  new_ext,
...
/* nifti_fill_extension  - given data and length, fill an extension struct
   ...
   return 0 on success, -1 on error (and free the entire list)
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_fill_extension( nifti1_extension *ext, const char * data,
                                int len, int ecode)
{
   int esize;
 ...
 *     error        : -1
 *----------------------------------------------------------------------*/
int vtknifti1_io::nifti_read_next_extension( nifti1_extension * nex, nifti_image *nim,
                                      int remain, znzFile fp )
{
   ...
   count = (int)vtkznzlib::znzread( &size, 4, 1, fp );
   if( count == 1 ) count += (int)vtkznzlib::znzread( &code, 4, 1, fp );

   if( count != 2 ){
      if( g_opts.debug > 2 )
         fprintf(stderr,"-d current extension read failed\n");
      vtkznzlib::znzseek(fp, -4*count, SEEK_CUR); /* back up past any read */
   ...
   if( !nifti_check_extension(nim, size, code, remain) ){
      if( vtkznzlib::znzseek(fp, -8, SEEK_CUR) < 0 ){      /* back up past any read */
   ...
   count = (int)vtkznzlib::znzread(nex->edata, 1, size, fp);
   if( count < size ){
      if( g_opts.debug > 0 )
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::valid_nifti_extensions(const nifti_image * nim)
{
   nifti1_extension * ext;
   int                c, errs;
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_is_valid_ecode( int ecode )
{
 ...
 *----------------------------------------------------------------------*/
int vtknifti1_io::nifti_check_extension(nifti_image *nim, int size, int code, int rem)
{
 ...
 * return NULL on failure
 *----------------------------------------------------------------------*/
znzFile vtknifti1_io::nifti_image_load_prep( nifti_image *nim )
{
   ...
   fp = vtkznzlib::znzopen(tmpimgname, "rb", nifti_is_gzfile(tmpimgname));
   if (znz_isnull(fp)){
   ...
   if( vtkznzlib::znzseek(fp , (long)ioff , SEEK_SET) < 0 ){
...
*/
int vtknifti1_io::nifti_image_load( nifti_image *nim )
{
   /* set up data space, open data file and seek, then call nifti_read_buffer */
   size_t ntot , ii ;
   znzFile fp ;
...
*//*--------------------------------------------------------------------*/
size_t vtknifti1_io::nifti_read_buffer(znzFile fp, void* dataptr, size_t ntot, 
                                nifti_image *nim)
{
  size_t ii;
...
*//*------------------------------------------------------------------------*/
void vtknifti1_io::nifti_image_unload( nifti_image *nim )
{
...
/*! free 'everything' about a nifti_image struct (including the passed struct)
    ...
    free (only fields which are not NULL):
      ...
      - nim
*//*------------------------------------------------------------------------*/
void vtknifti1_io::nifti_image_free( nifti_image *nim )
{
   if( nim == NULL ) return ;
...
*//*------------------------------------------------------------------------*/
int vtknifti1_io::nifti_free_extensions( nifti_image *nim )
{
   int c ;
...
*//*------------------------------------------------------------------------*/
void vtknifti1_io::nifti_image_infodump( const nifti_image *nim )
{
...
*/
size_t vtknifti1_io::nifti_write_buffer(znzFile fp, const void *buffer, size_t numbytes)
{
   ...
   ss = vtkznzlib::znzwrite( (const void*)buffer , 1 , numbytes , fp ) ;
   return ss;
}
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_write_all_data(znzFile fp, nifti_image * nim,
                         const nifti_brick_list * NBL)
{
   size_t ss;
   int    bnum;
...
int vtknifti1_io::nifti_write_extensions(znzFile fp, nifti_image *nim)
{
...
*//*--------------------------------------------------------------------*/
nifti_image* vtknifti1_io::nifti_simple_init_nim(void)
{
  nifti_image *nim;
  struct nifti_1_header nhdr;
  int nbyper, swapsize;
...
*//*--------------------------------------------------------------------*/
nifti_1_header * vtknifti1_io::nifti_make_new_header(const int arg_dims[], int arg_dtype)
{
   nifti_1_header * nhdr;
...
*//*--------------------------------------------------------------------*/
nifti_image * vtknifti1_io::nifti_make_new_nim(const int dims[], int datatype, int data_fill)
{
   nifti_image    * nim;
   nifti_1_header * nhdr;
...
*//*--------------------------------------------------------------------*/
nifti_1_header vtknifti1_io::nifti_convert_nim2nhdr(const nifti_image * nim)
{
   struct nifti_1_header nhdr;
...
*/
int vtknifti1_io::nifti_copy_extensions(nifti_image * nim_dest, const nifti_image * nim_src)
{
   char   * data;
   size_t   bytes;
   int      c, size, old_size;
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_extension_size(nifti_image *nim)
{
   int c, size = 0;
...
*//*--------------------------------------------------------------------*/
void vtknifti1_io::nifti_set_iname_offset(nifti_image *nim)
{
   int offset;
...
/*! write the nifti_image dataset to disk, optionally including data
   ...
   \sa nifti_image_write, nifti_image_write_hdr_img2, nifti_image_free,
       nifti_set_filenames
*//*--------------------------------------------------------------------*/
znzFile vtknifti1_io::nifti_image_write_hdr_img( nifti_image *nim , int write_data , 
                                          const char* opts )
{
 ...
 *     nifti_set_filenames
*//*---------------------------------------------------------------------*/
znzFile vtknifti1_io::nifti_image_write_hdr_img2(nifti_image *nim, int write_opts, 
               const char * opts, znzFile imgfile, const nifti_brick_list * NBL)
{
   ...
   if( ! znz_isnull(imgfile) && nim->nifti_type == NIFTI_FTYPE_NIFTI1_1 ){
      ...
      fp = vtkznzlib::znzopen( nim->fname , opts , nifti_is_gzfile(nim->fname) ) ;
      if( znz_isnull(fp) ){
   ...
   if( ss < sizeof(nhdr) ){
      ...
      vtkznzlib::znzclose(fp); return fp;
   }
   ...
   if( nim->nifti_type != NIFTI_FTYPE_NIFTI1_1 ){ /* get a new file pointer */
      ...
      if( ! znz_isnull(imgfile) ){
         ...
         fp = vtkznzlib::znzopen( nim->iname , opts , nifti_is_gzfile(nim->iname) ) ;
         if( znz_isnull(fp) ) ERREX("cannot open image file") ;
      }
   }

   vtkznzlib::znzseek(fp, nim->iname_offset, SEEK_SET);  /* in any case, seek to offset */
...
*//*--------------------------------------------------------------------*/
znzFile vtknifti1_io::nifti_write_ascii_image(nifti_image *nim, const nifti_brick_list * NBL,
                              const char *opts, int write_data, int leave_open)
{
   ...
   vtkznzlib::znzputs(hstr,fp);                                               /* header */
...
*//*------------------------------------------------------------------------*/
void vtknifti1_io::nifti_image_write( nifti_image *nim )
{
...
*//*--------------------------------------------------------------------*/
void vtknifti1_io::nifti_image_write_bricks( nifti_image *nim, const nifti_brick_list * NBL )
{
...
*//*--------------------------------------------------------------------*/
nifti_image * vtknifti1_io::nifti_copy_nim_info(const nifti_image * src)
{
  nifti_image *dest;
...
int vtknifti1_io::unescape_string( char *str )
{
   int ii,jj , nn,ll ;
...
char *vtknifti1_io::escapize_string( const char * str )
{
   int ii,jj , lstr,lout ;
   char *out ;
...
*//*-------------------------------------------------------------------------*/
char *vtknifti1_io::nifti_image_to_ascii( const nifti_image *nim )
{
...
*//*--------------------------------------------------------------------*/
int vtknifti1_io::nifti_short_order(void)   /* determine this CPU's byte order */
{
...
*//*-------------------------------------------------------------------------*/
int vtknifti1_io::nifti_nim_is_valid(nifti_image * nim, int complain)
{
   int errs = 0;
...
*//*-------------------------------------------------------------------------*/
int vtknifti1_io::nifti_nim_has_valid_dims(nifti_image * nim, int complain)
{
   size_t prod;
   int    c, errs = 0;
...
/*! read a nifti image, collapsed across dimensions according to dims[8]  <pre>
    ...
    \sa nifti_image_read, nifti_image_free, nifti_image_read_bricks
        nifti_image_load
*//*-------------------------------------------------------------------------*/
int vtknifti1_io::nifti_read_collapsed_image( nifti_image * nim, const int dims [8],
                                void ** data )
{
   znzFile fp;
   ...
   c = rci_read_data(nim, pivots,prods,nprods,dims,
                     (char *)*data, fp, vtkznzlib::znztell(fp));
   vtkznzlib::znzclose(fp);   /* in any case, close the file */
...
** stride array.
*/
void vtknifti1_io::compute_strides(int *strides,const int *size,int nbyper)
{
  int i;
  strides[0] = nbyper;
  for(i = 1; i < 7; i++)
...
*//*-------------------------------------------------------------------------*/
int vtknifti1_io::nifti_read_subregion_image( nifti_image * nim,
                                int *start_index,
                                int *region_size,
                                void ** data )
{
  ...
  */
   ...
   */
  initial_offset = vtkznzlib::znztell(fp);  
  /* get strides*/
          ...
          for(m = si[2]; m < (si[2] + rs[2]); m++)
            ...
            for(n = si[1]; n < (si[1] + rs[1]); n++)
              ...
              offset = initial_offset +
                ...
                (n * strides[1]) +
                (si[0] * strides[0]);
              vtkznzlib::znzseek(fp, offset, SEEK_SET); /* seek to current row */
...
*/
int vtknifti1_io::rci_read_data(nifti_image * nim, int * pivots, int * prods,
      int nprods, const int dims[], char * data, znzFile fp, size_t base_offset)
{
   ...
   if( nprods == 1 ){
      ...
      vtkznzlib::znzseek(fp, (long)base_offset, SEEK_SET);
...
*/
int vtknifti1_io::rci_alloc_mem(void ** data, int prods[8], int nprods, int nbyper )
{
   int size, memindex;
...
*/
int vtknifti1_io::make_pivot_list(nifti_image * nim, const int dims[], int pivots[],
...
*//*-------------------------------------------------------------------*/
int * vtknifti1_io::nifti_get_intlist( int nvals , const char * str )
{
   int *subv = NULL ;
...
*//*-------------------------------------------------------------------*/
int vtknifti1_io::nifti_datatype_from_string( const char * name )
{
...
*//*-------------------------------------------------------------------*/
const char * vtknifti1_io::nifti_datatype_to_string( int dtype )
{
    int tablen = sizeof(nifti_type_list)/sizeof(nifti_type_ele);
    int c;
...
*//*-------------------------------------------------------------------*/
int vtknifti1_io::nifti_datatype_is_valid( int dtype, int for_nifti )
{
    int tablen = sizeof(nifti_type_list)/sizeof(nifti_type_ele);
    int c;
...
*//*-------------------------------------------------------------------*/
int vtknifti1_io::nifti_test_datatype_sizes(int verb)
{
    int tablen = sizeof(nifti_type_list)/sizeof(nifti_type_ele);
    int nbyper, ssize;
    int c, errs = 0;
 ...
 *  else           : display all
*//*-------------------------------------------------------------------*/
int vtknifti1_io::nifti_disp_type_list( int which )
{
    const char * style;
    int    tablen = sizeof(nifti_type_list)/sizeof(nifti_type_ele);
    int    lwhich, c;
