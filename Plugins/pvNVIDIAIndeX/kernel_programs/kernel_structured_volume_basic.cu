...
// # Summary:
// A simple volume kernel for structure volumes.
// It samples the volume scalar value for the current position and
// maps it to color through the colormap. This is very similar to what
// IndeX does by default when not Visual ELements are used.
