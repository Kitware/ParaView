...
///
class IConfig_settings : public mi::base::Interface_declare<0xf7d37400, 0xdf43, 0x49f2, 0x8e, 0x12,
                           0x67, 0xe3, 0x62, 0x90, 0xad, 0xee, mi::neuraylib::IElement>
{
public:
  ...
  ///
  /// \return The current level-of-detail height-field renderer configuration (c.f., \c
  /// Height_field_lod_config).
  ///
  ...
  ///
  /// \param[in] initial_subcube_size     The initial subcube size to use. It actually used size
  /// might
  ///                                     become smaller is rotation or scaling is enabled.
  ///                                     (default: 510)
  /// \param[in] subcube_border_size      Controls the size of the border around each subcube, for
  /// which
  ///                                     neighboring voxel data is stored. The default border size
  ///                                     is 1,
  ///                                     which makes trilinear filtering possible. For more complex
  ///                                     filters (using larger kernels) a larger border size may be
  ///                                     required.
  /// \param[in] support_continuous_volume_translation  Controls whether a translation of volume
  /// scene elements
  ///                                     should be supported for arbitrary float values (true) or
  ///                                     just for integer values (false). Since enabling this
  ///                                     feature reduces the
  ///                                     usable subcube size, it should only be enabled when
  ///                                     needed.
  ///                                     (default: false)
  /// \param[in] support_volume_rotation  Controls whether rotation of volume scene elements
  ///                                     should be supported. Since enabling this feature reduces
  ///                                     the
  ///                                     usable subcube size, it should only be enabled when
  ///                                     needed.
  ///                                     (default: false)
  /// \param[in] minimal_volume_scaling   The minimum scaling factor for volume scene elements that
  ///                                     should be supported. Since any value less than 1 reduces
  ///                                     the
  ///                                     usable subcube size, the scaling factor should be chosen
  ///                                     only
  ///                                     as small as absolutely needed. The default of (1, 1, 1)
  ///                                     should be used if no scaling is required. Values given
  ///                                     here
  ///                                     must be greater than 0.0 and less or equal to 1.0. When a
  ///                                     scene element is scaled by a negative scaling factor then
  ///                                     the
  ///                                     absolute value of this factor should be used to determine
  ///                                     the
  ...
  virtual bool set_subcube_configuration(
    ...
    mi::Uint32 subcube_border_size, bool support_continuous_volume_translation,
  ...
  ///
  virtual void get_minimal_volume_scaling(
    mi::math::Vector_struct<mi::Float32, 3>& minimal_scaling) const = 0;
  ...
  virtual void set_heightfield_normal_calculation_mode(
    Heightfield_normal_calculation_mode calculation_mode) = 0;
