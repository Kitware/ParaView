...
///
class ICompute_launch_request : public mi::base::Interface_declare<0x7487d3dd, 0x72e3, 0x4a9e, 0x9e,
                                  0x9d, 0x1d, 0x8b, 0xfb, 0x96, 0xd5, 0xa0>
{
public:
...
class ICompute_plane_parameters
  : public mi::base::Interface_declare<0xdaf77e98, 0xd880, 0x415c, 0x86, 0x39, 0x2d, 0x6b, 0xee,
      0x9e, 0x5c, 0xbe, nv::index::IAttribute>
{
public:
  /// Define surface parameterization of the plane. This specifies the resolution of the 2D result
  /// buffer.
...
class ICompute_result_buffer_2D
  : public mi::base::Interface_declare<0x8f335887, 0x6c3b, 0x48be, 0xb0, 0x55, 0x46, 0xb0, 0xc4,
      0xdb, 0xf4, 0xb0, IDistributed_data_subset>
{
public:
  /// Get the resolution of this buffer. Note that this might be smaller than the resolution
  /// of \c ICompute_plane_parameters, depending on the bounding box given to \c
  /// IDistributed_data_access.
  virtual mi::math::Vector_struct<mi::Uint32, 2> get_resolution() const = 0;
  ...
  /// Get value mask. The bits of a value mask tell if a user value was written in the XAC compute
  /// program.
  /// If the bit is 0, then the value with value index equivalent to the bit number is not valid.
  ///
  /// \return Pointer to mask data with resolution dependent layout (x-major), or 0 if buffer is not
  /// valid.
  ...
  /// \return Pointer to values with resolution dependent layout (x-major), or 0 if buffer is not
  /// valid.
