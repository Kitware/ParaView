...
public:
  ...
  inline explicit Bbox(const Vector& point) ///< point.
  {
    min = point;
    max = point;
  }
  ...
  inline Bbox(const Vector& nmin, ///< \c min corner vector
  ...
  ///
  /// \pre <tt>DIM == 1</tt>
  inline Bbox(T min_x, ///< x-coordinate of \c min corner vector
  ...
  inline Bbox add_motionbox(const Bbox& vbox, T t) const
  {
  ...
  /// boxes.
  inline void push_back(const Bbox& other) { return insert(other); }
...
template <typename T, Size DIM>
inline Bbox<T, DIM> lerp(const Bbox<T, DIM>& bbox1, ///< one bounding box
...
template <typename T, Size DIM>
inline Bbox<T, DIM> clip(const Bbox<T, DIM>& bbox1, ///< first bounding box
  const Bbox<T, DIM>& bbox2)                        ///< second bounding box
{
  Bbox<T, DIM> result(Bbox<T, DIM>::UNINITIALIZED_TAG);
  for (Size i = 0; i < DIM; i++)
...
void Bbox<T, DIM>::robust_grow(T eps)
{
  ...
  Vector dist;
  for (Size i = 0; i < DIM; i++)
    dist[i] = base::abs(min[i]) + base::abs(max[i]) + base::abs(max[i] - min[i]);
