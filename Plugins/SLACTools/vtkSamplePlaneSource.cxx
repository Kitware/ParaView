...
//-----------------------------------------------------------------------------
int vtkSamplePlaneSource::FillInputPortInformation(int port,
                                                   vtkInformation *info)
{
  if (port == 0)
    {
...
//-----------------------------------------------------------------------------
void vtkSamplePlaneSource::ComputeLocalBounds(vtkDataObject *input,
                                              double bounds[6])
{
  ...
  if (cInput)
    ...
    for (iter->InitTraversal(); !iter->IsDoneWithTraversal();
      ...
      if (bounds[0] > subbounds[0]) bounds[0] = subbounds[0];
      if (bounds[1] < subbounds[1]) bounds[1] = subbounds[1];
      if (bounds[2] > subbounds[2]) bounds[2] = subbounds[2];
      if (bounds[3] < subbounds[3]) bounds[3] = subbounds[3];
      if (bounds[4] > subbounds[4]) bounds[4] = subbounds[4];
      if (bounds[5] < subbounds[5]) bounds[5] = subbounds[5];
      }
    return;
    }
...
//-----------------------------------------------------------------------------
void vtkSamplePlaneSource::CreatePlane(const double bounds[6],
                                       vtkPolyData *output)
{
  double dims[3];
  for (int i = 0; i < 3; i++)
