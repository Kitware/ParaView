...
void ConfigureMapper( vtkPolyDataMapper * mapper, 
                      double              scalarMin,
                      double              scalarMax, 
...
void ConfigureProperty( vtkProperty * property,
                        int           shading,
...
void DumpColorMapInfo( vtkPolyDataMapper * mapper, vtkProperty * property,
                       int vectorByVTKmapper );
...
# endif
  ...
  ConfigureColorLUT( sphereLUT, 
                     0.0, 1.0,                         // scalar range
                     0.00, 0.6,                        // hue    range
...
# ifdef _MULTI_OBJECTS_
  ...
  #ifdef _DUMPOUT_INFOR_
  DumpColorMapInfo( cylinderMapper, cylinderProperty, 
                    cylinderVectorByVTKmapper );
  #endif
...
# endif
  ...
  if ( retVal == vtkRegressionTester::DO_INTERACTOR )
    ...
    for ( int colorLutVectorMode = 0;              // 0 MAGNITUDE
              ...
              colorLutVectorMode ++ )
    for ( int mapperScalarVisibility = 0;
              mapperScalarVisibility < 2;
              mapperScalarVisibility ++ )
    ...
    for ( int propertyColor = 0;
              propertyColor < 2;
              propertyColor ++ )
      {
      ConfigureMapper( sphereMapper,
                       -1.0, 1.0,                  // scalar range
                      ...
                      int                 vectorByVTKmapper )
{
  ...
  if ( vectorByVTKmapper )
    ...
    // 
    mapper->ColorByArrayComponent( colorByArrayName, 
                                   colorByArrayVectorComponentIndex );
    }
  else
    {
...
void DumpAttributeInfo( vtkPolyData * polyData, char * objName )
{
  ...
  if ( cellData )
    {
    cerr << endl << objName << " cell  data has " 
                 << cellData->GetNumberOfArrays() << " array(s)" << endl;

    for ( i = 0; i < cellData->GetNumberOfArrays(); i ++ )
      {
      cerr << "  array #" << i << " = " 
           << cellData->GetArrayName( i ) << endl;
      }
    }
  if ( pointData )
    {
    cerr << endl << objName << " point data has "
                 << pointData->GetNumberOfArrays() << " array(s)" << endl;
    for ( i = 0; i < pointData->GetNumberOfArrays(); i ++ )
      {
      cerr << "  array #" << i << " = " 
           << pointData->GetArrayName( i ) << endl;
      }
    }
...
{
  vtkLookupTable * lut 
    = vtkLookupTable::SafeDownCast( mapper->GetLookupTable() );
  
  cerr << endl << "Mapper Information"
       << endl << "  Scalar Visibility    = "
               << mapper->GetScalarVisibility()
               << " (whether or not scalar data is used for coloring)"
       << endl << "  Scalar Mode          = "  << mapper->GetScalarMode()
               << " ( Default = 0;  PointData = 1;  CellData = 2; " 
               << " PointFieldData = 3;  CellFieldData = 4;  FieldData = 5 )"
       << endl << "  Scalar Data Range    = (" << mapper->GetScalarRange()[0]
               << ", "                       << mapper->GetScalarRange()[1]
               << ")"
       << endl << "  Array Name           = " << mapper->GetArrayName()
       << endl << "  Array Component      = " << mapper->GetArrayComponent()
               << " (only upon vector array used for color mapping)"
       << endl << "  vector by vtkMapper  = " << vectorByVTKmapper
               << " (vector array handled by vtkMapper or vtkLookupTable)"
       << endl << "  Scalar Material Mode = "
               << mapper->GetScalarMaterialModeAsString()
               << " ( Default / Ambient / Diffuse / AmbientAndDiffuse )"
       << endl << "  Color  Mode          = "  << mapper->GetColorModeAsString()
               << " ( Default <unsigned char scalars directly used as colors> /"
               << " MapScalars <through color LUT> )" << endl 
       << endl << "  Use Lookup Table Scalar Range   = "
               << mapper->GetUseLookupTableScalarRange()
       << endl << "  InterpolateScalarsBeforeMapping = "
               << mapper->GetInterpolateScalarsBeforeMapping()
       << endl << "  Geometric Data Bounding Box     = "
               << mapper->GetBounds()[0] << " <= x <= " 
               << mapper->GetBounds()[1] << ";  "
               << mapper->GetBounds()[2] << " <= y <= " 
               << mapper->GetBounds()[3] << ";  "
               << mapper->GetBounds()[4] << " <= z <= " 
               << mapper->GetBounds()[5] << endl


       << endl << "Lookup Table Information (accessed through mapper)" << endl
               << "  LUT scalar Range = (" << lut->GetRange()[0] 
               << ", "                     << lut->GetRange()[1] << ")"
               << " --- possibly different from the scalar DATA range"
       << endl << "  Table Range      = (" << lut->GetTableRange()[0]
               << ", "                     << lut->GetTableRange()[1] << ")"
               << " --- exactly the same as LUT Scalar Range"
       << endl << "  Hue    Range     = (" << lut->GetHueRange()[0]
               << ", "                     << lut->GetHueRange()[1] << ")"
       << endl << "  Alpha Value      = "  << lut->GetAlpha()
       << endl << "  Alpha Range      = (" << lut->GetAlphaRange()[0]
               << ", "                     << lut->GetAlphaRange()[1] << ")"
       << endl << "  Value Range      = (" << lut->GetValueRange()[0]
               << ", "                     << lut->GetValueRange()[1] << ")"
       << endl << "  Number of Colors = "  << lut->GetNumberOfColors()
       << endl << "  Scale Mode       = "  << lut->GetScale()
               << " ( VTK_SCALE_LINEAR = 0 / VTK_SCALE_LOG10 = 1 )"
       << endl << "  Vector Mode      = "  << lut->GetVectorMode()
               << " ( Magnitude = 0;  Component = 1 <default> )"
       << endl << "  Vector Component = "  << lut->GetVectorComponent()
               << " (only upon vector array used for color mapping)" << endl


       << endl << "Property Information"
       << endl << "  Shading             = "  << property->GetShading()
       << endl << "  RGB Color           = (" << property->GetColor()[0]
       << ", " << property->GetColor()[1] << ", "
               << property->GetColor()[2] << ")"
       << endl << "  Ambient Coefficient = "  << property->GetAmbient()
       << endl << "  Diffuse Coefficient = "  << property->GetDiffuse()
       << endl << "  Specular Coeficient = "  << property->GetSpecular()
       << endl << "  Specular Power      = "  << property->GetSpecularPower()
     //<< endl << "  Material Name       = "  << property->GetMaterialName()
       << endl << "  Edge Visibility     = "  << property->GetEdgeVisibility()
               << " (Wireframe does not implicitly set Edge Visibility on)"
       << endl << "  Interpolation Mode  = " 
               << property->GetInterpolationAsString()
               << " ( Flat / Gouraud / Phong )"
       << endl << "  Representation Mode = " 
               << property->GetRepresentationAsString()
