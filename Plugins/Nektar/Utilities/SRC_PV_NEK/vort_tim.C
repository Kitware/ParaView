 ...
 *                        RCS Information                                    *
 *                                                                           *
 * $Source: /homedir/cvs/Nektar/Utilities/src/vort_tim.C,v $
 * $Revision: 1.2 $
...
int  setup (FileList *f, Element_List **U, int *nft){
  ...
  nftot   = nfields;
#ifdef TIM
  nftot  += (fld.dim == 3) ? 4:4; //only changed 2D
#else
  nftot  += (fld.dim == 3) ? 4:2;
#endif
...
#ifdef TIM
    double **dp = dmatrix(0,3,0,QGmax*QGmax-1);
    double *Upx = dp[0], *Upy = dp[1];
    double *Vpx = dp[2], *Vpy = dp[3];
#endif
    {
      for(k = 0; k < U[0]->nel; ++k){
...
#ifdef TIM
  U[3]->flist[k]->Grad_d(Upx, Upy, 0, 'a');
  U[4]->flist[k]->Grad_d(Vpx, Vpy, 0, 'a');
#endif
  dvsub(qt, Vx, 1, Uy, 1, *U[nfields]->flist[k]->h, 1);
  dvadd(qt, Ux, 1, Vy, 1, *U[nfields+1]->flist[k]->h, 1);
#ifdef TIM
  vsub(qt, Vpx, 1, Upy, 1, *U[nfields+2]->flist[k]->h, 1);
  dvadd(qt, Upx, 1, Vpy, 1, *U[nfields+3]->flist[k]->h, 1);
#endif
...
static void WriteS(Element_List **E, FILE *out, int nfields){
  ...
  for(n = 0; n < nfields; ++n){
    ...
    Interp_symmpts(F,qa,F->h[0],F->h[0],'p');
        ...
        if(Z = dparam("Z_SLICE")){
          for(i = 0; i < ntot; ++i){
            fprintf(out,"%lg %lg %lg", X.x[i], X.y[i],Z);
            for(n = 0; n < nfields; ++n)
...
static void Write(Element_List **E, FILE *out, int nfields){
  ...
  double   *z,*w, ave, xmove=0.0,ymove=0.0,tmove=0.0,Z;
  Coord    X;
  char     *outformat;
  Element  *F;
  ...
  fprintf(out,"VARIABLES = x y");
      ...
      for(i = 0; i < nfields-4; ++i)
  fprintf(out," %c", E[i]->fhead->type);
      fprintf(out," Vorticity Divergence PVorticity PDivergence");
#else
      for(i = 0; i < nfields-2; ++i)
  fprintf(out," %c", E[i]->fhead->type);
      fprintf(out," Vorticity Divergence");
  ...
  if(Z=dparam("Z_SLICE"))
    for(i = 0; i < F->qtot; ++i){
      fprintf(out,"%lg %lg %lg", X.x[i]+xmove, X.y[i]+ymove,Z);
      for(n = 0; n < nfields; ++n)
        fprintf(out," %lg",E[n]->flist[k]->h[0][i]);
      fputc('\n',out);
    }
