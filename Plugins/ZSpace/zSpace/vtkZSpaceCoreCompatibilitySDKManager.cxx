...
vtkZSpaceCoreCompatibilitySDKManager::~vtkZSpaceCoreCompatibilitySDKManager()
{
  ...
  error = this->EntryPts.zccompatDestroyViewport(this->ViewportHandle);
  ZSPACE_CHECK_ERROR(zccompatDestroyViewport, error);
...
{
  auto* rw = vtkGenericOpenGLRenderWindow::SafeDownCast(renderWindow);
  if (!rw)
  {
    vtkErrorMacro("Unable to set the zSpace Core Compatbility SDK Render Window:"
                  "unable to cast it to vtkGenericOpenGLRenderWindow.");
    return;
  }
