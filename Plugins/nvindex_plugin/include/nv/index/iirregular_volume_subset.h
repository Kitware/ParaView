...
///
/// An irregular volume dataset is defined as a set of cells (see the \c
/// IIrregular_volume_subset::Cell structure). The
/// cells are defined by a set of faces (see the \c IIrregular_volume_subset::Face structure). The
/// faces, finally, are
/// defined by a set of vertex positions in three dimensional space. The cell, face and vertex
/// information is handled in
/// three separate arrays (see \c IIrregular_volume_subset::Mesh_storage). In order to support
/// irregular volumes with
/// heterogeneous cell and face definitions, which allows for cells with varying numbers of faces
/// and faces with varying
/// numbers of vertices, the association of faces to cells and vertices to faces is handled by
/// shared cell-face and
/// face-vertex arrays. This further allows for sharing faces of connected cells in the irregular
/// volume mesh.
///
/// Attribute values can either be defined per vertex or per cell (see
/// IIrregular_volume_subset::Attribute_affiliation).
///
...
class IIrregular_volume_subset
  : public mi::base::Interface_declare<0xe00304d7, 0x3f25, 0x4719, 0xb4, 0x7e, 0x3d, 0x10, 0xec,
      0xbd, 0x3c, 0x2f, IDistributed_data_subset>
{
public:
  ...
  /// index of the face vertices in the shared face-vertex array (see \c Mesh_storage). The
  /// face-vertex indices are
  ...
  ///
  /// This structure defines the basic irregular volume dataset parameters such as the number of
  /// vertices, faces and
  /// cells in the underlying mesh structure as well as the size of the shared cell-face and
  /// face-vertex arrays.
  ///
  /// An additional global mesh maximum edge length is a required mesh parameter the rendering
  /// system strongly
  /// requires. This parameter cannot be determined in a straightforward way inside the rendering
  /// system as
  ...
  struct Mesh_parameters
  {
    ...
    mi::Float32
      global_max_edge_length; ///< The length of the longest edge in the irregular volume mesh
  };

  /// Irregular volume mesh storage.
  ///
  /// The storage is generated by a call to \c generate_mesh_storage() passing an initialized
  /// instance of mesh parameters
  /// \c Mesh_parameters(). A valid mesh storage then contains initialized arrays for storing the
  /// irregular volume
  /// subset.
  ///
  struct Mesh_storage
  ...
  ///
  /// \note The currently only supported attribute affiliation allows for assigning attribute values
  /// to irregular
  ...
  ///
  /// The currently only supported attribute type is an 8bit unsigned integer type. Future types
  /// will encompass
  /// different single channel as well as multi-channel attribute values with higher precision base
  /// types.
  ///
  enum Attribute_type
  {
  ...
  ///
  /// This structure defines the basic parameters of a single attribute set associated with the
  ...
  /// Attribute storage.
  ///
  /// The attribute storage is generated by a call to \c generate_mesh_storage() passing an
  /// initialized instance of
  /// attribute parameters \c Attribute_parameters. A valid attribute storage contains a raw pointer
  /// to an array of
  /// attribute values.
  ///
  ...
  ///
  /// This function initializes an instance of \c Mesh_storage according to the passed instance of
  /// \c Mesh_parameters.
  /// At any point during the lifetime of an \c IIrregular_volume_subset instance there can only be
  /// a single valid
  /// mesh storage. Multiple calls to this function will invalidate previous instances of the mesh
  /// storage and return
  /// a new valid instance according the newly passed mesh parameters.
  ///
  /// \param[in]  mesh_params     The irregular volume mesh parameters.
  /// \param[out] mesh_storage    Mesh_storage instance that contains valid references to the
  /// vertex,
  ...
  virtual bool generate_mesh_storage(
    const Mesh_parameters& mesh_params, Mesh_storage& mesh_storage) = 0;

  /// Generate and initialize an instance of irregular volume attribute set storage.
  ///
  /// This function initializes an instance of \c Attribute_storage according to the passed instance
  /// of \c Attribute_parameters.
  /// An irregular volume dataset can be associated with multiple attribute sets which are
  /// identified by an attribute
  /// index. At any point during the lifetime of an \c IIrregular_volume_subset instance there can
  /// only be a single valid
  /// attribute storage per attribute index. Multiple calls to this function will invalidate
  /// previous instances of the
  /// attribute storage associated with the passed index value and return a new valid instance
  /// according the newly
  ...
  /// \param[out] attrib_storage  Attribute storage instance that contains valid references to the
  /// attribute value array.
  ///
  /// \return                     True if a valid storage was generated, false otherwise.
  ///
  virtual bool generate_attribute_storage(mi::Uint32 attrib_index,
    const Attribute_parameters& attrib_params, Attribute_storage& attrib_storage) = 0;

  /// Get the irregular volume mesh parameters of the currently valid mesh storage.
  ///
  /// When this function is called before initializing a valid mesh storage (\c
  /// generate_mesh_storage()) the returned
  /// instance of \c Mesh_parameters will describe an empty irregular volume mesh (i.e. all values
  /// 0).
  ///
  ...
  /// When this function is called before initializing a valid mesh storage (\c
  /// generate_mesh_storage()) the returned
  /// instance of \c Mesh_storage will hold invalid pointers to the particular arrays (i.e. value
  /// 0).
  ///
  ...
  ///
  /// \return                     True when the attribute set according to the passed index could be
  ...
  ///
  /// \return                     True when the attribute storage according to the passed index
