## TestAdaptive.py
## Test Adaptive Paraview
##
## Usage: pvbatch testAdaptive.py --plugin <plugin-filename> [OPTIONS]
##
#########################################################################
...
adaptivePlugin = options.plugin

if (len(adaptivePlugin) == 0):
  print "Please specify path to adaptive plugin using --plugin <filename>."
  sys.exit(1)
##########################################################################
...
## Load plugin
servermanager.LoadPlugin(adaptivePlugin)

## Create adaptive streaming view
view = servermanager.rendering.RenderView()
view.StillRender()
view = servermanager.rendering.AdaptiveRenderView()
